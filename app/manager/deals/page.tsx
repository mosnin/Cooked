...
export const metadata: Metadata = { title: 'Deals — Team' };
 ...
 *
 * Because each rep can have custom stage names, we build columns from
 ...
 *     stage?" when reps name them the same.
 ...
 * rep pipeline direction even across mixed stage names.
 */
function buildColumns(
  stages: StageRow[],
  dealsWithRep: Array<ManagerDealItem & { stageId: string }>,
): ManagerKanbanColumn[] {
  ...
  for (const deal of dealsWithRep) {
...
export default async function ManagerDealsPage() {
  ...
  // Resolve ALL team members — same pattern as the previous page.
  ...
  // Build spaceId to rep name lookup
  ...
  const statusSentence = (() => {
    if (activeCount === 0) return 'No active deals across your team yet.';
    const parts: string[] = [];
    parts.push(
