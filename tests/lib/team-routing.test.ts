/**
 * BP7d — vitest coverage for the team lead-routing engine (BP7b).
 *
 * The engine under test is `routeTeamLead(teamId)` in
 * lib/team-routing.ts. Three sibling agents (BP7a schema, BP7b
 ...
 * `.maybeSingle()` on the Team row for config load. Our `singleThen`
...
// `updateCalls.Team` in particular.
...
function makeTeam(overrides: Partial<TeamConfigRow> = {}): TeamConfigRow {
  return {
...
/**
 * Seed a canonical "three eligible reps" fixture:
 *   agents u_a, u_b, u_c, each with a Space inside team b_1.
 * Tests that care about a different topology override `mockByTable`
 * entries directly.
 */
function seedThreeAgents(
  config: Partial<TeamConfigRow> = {},
  opts: { extraUsers?: Array<Record<string, unknown>> } = {},
): void {
  mockByTable.Team = { single: makeTeam(config) };
  mockByTable.TeamMembership = {
    rows: [
      { userId: 'u_a', role: 'rep_member' },
      { userId: 'u_b', role: 'rep_member' },
      { userId: 'u_c', role: 'rep_member' },
    ],
  };
  mockByTable.User = {
    rows: [
  ...
  mockByTable.Space = {
    rows: [
      { id: 's_a', ownerId: 'u_a', teamId: 'b_1' },
      { id: 's_b', ownerId: 'u_b', teamId: 'b_1' },
      { id: 's_c', ownerId: 'u_c', teamId: 'b_1' },
    ],
  };
}

async function callRouter(): Promise<unknown> {
  const mod = await import('@/lib/team-routing');
  return mod.routeTeamLead('b_1');
}
...
// Gating
// ──────────────────────────────────────────────────────────────────────────
describe('routeTeamLead — gating', () => {
  it('returns null when autoAssignEnabled is false', async () => {
    mockByTable.Team = {
      single: makeTeam({ autoAssignEnabled: false, assignmentMethod: 'round_robin' }),
    };
    const result = await callRouter();
    expect(result).toBeNull();
    // No side-effect write to Team.
    expect(updateCalls.Team ?? []).toHaveLength(0);
  });

  it("returns null when assignmentMethod is 'manual'", async () => {
    mockByTable.Team = {
      single: makeTeam({ autoAssignEnabled: true, assignmentMethod: 'manual' }),
    };
    const result = await callRouter();
    expect(result).toBeNull();
    expect(updateCalls.Team ?? []).toHaveLength(0);
  ...
  it('returns null when no rep_members exist', async () => {
    mockByTable.Team = {
      single: makeTeam({ autoAssignEnabled: true, assignmentMethod: 'round_robin' }),
    };
    mockByTable.TeamMembership = { rows: [] };
  ...
  it('returns null when the only rep has no Space in this team', async () => {
    mockByTable.Team = {
      single: makeTeam({ autoAssignEnabled: true, assignmentMethod: 'round_robin' }),
    };
    mockByTable.TeamMembership = {
      rows: [{ userId: 'u_orphan', role: 'rep_member' }],
    };
    mockByTable.User = { rows: [{ id: 'u_orphan', status: 'active' }] };
    // Space exists, but the engine filters by `teamId=this` — returning an
    // empty `rows` for Space is the simplest way to simulate a cross-tenant
    // miss because the chain mock doesn't actually filter by `eq('teamId')`.
    mockByTable.Space = { rows: [] };
    const result = await callRouter();
    expect(result).toBeNull();
    expect(updateCalls.Team ?? []).toHaveLength(0);
  ...
  it('fails closed on infra errors (Team select errors → null, no update)', async () => {
    mockByTable.Team = {
...
// Round-robin
// ──────────────────────────────────────────────────────────────────────────
describe('routeTeamLead — round_robin', () => {
  ...
  it('ignores offboarded agents (eligibility filter)', async () => {
    ...
    mockByTable.Team = {
      single: makeTeam({ assignmentMethod: 'round_robin', lastAssignedUserId: null }),
    };
    mockByTable.TeamMembership = {
      rows: [
        { userId: 'u_a', role: 'rep_member' },
...
// Score-based
// ──────────────────────────────────────────────────────────────────────────
describe('routeTeamLead — score_based', () => {
  /**
