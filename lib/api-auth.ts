 ...
 *
 * Team offboarding status gate: after Clerk auth succeeds we look up the
 ...
 * team; their book of business has been reassigned and they must lose API
  ...
  try {
    ...
    if (userRow && (userRow as { status?: string }).status === 'offboarded') {
      return NextResponse.json(
        { error: 'Your access has been revoked by your team.', code: 'offboarded' },
        { status: 403 },
      );
    }
  } catch {
 ...
 * manager_owner/manager_admin of the team that manages this space.
...
export async function requireSpaceOwner(
  ...
  // Manager owner/admin check — allow managing team members' spaces
  ...
  if (dbUser) {
    // Check if the space belongs to a team the user is admin/owner of.
    // Fetch ALL manager-level memberships rather than .maybeSingle() — a user
    // who owns/admins more than one team would otherwise make
    // .maybeSingle() throw (PostgREST errors on >1 row), 500ing a legitimate
    // multi-team admin. Mirror the context helpers in lib/permissions.ts:
    // fetch all, then deterministically prefer manager_owner over manager_admin.
    const { data: memberships } = await supabase
      .from('TeamMembership')
      .select('role, teamId, createdAt')
      .eq('userId', dbUser.id)
      .in('role', ['manager_owner', 'manager_admin'])
      .order('createdAt', { ascending: true });

    // The caller may manager-own/admin MORE THAN ONE team. Grant access
    ...
    // team, so e.g. a manager_owner of A who is also manager_admin of B was
    // wrongly 403'd when opening a space owned by a B member.
    const managerTeamIds = (memberships ?? []).map((m) => m.teamId);

    if (managerTeamIds.length > 0) {
      const { data: spaceOwnerMembership } = await supabase
        .from('TeamMembership')
        .select('id')
        .in('teamId', managerTeamIds)
