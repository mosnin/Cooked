...
export interface ResponseShape {
  ...
  generatedAt: string;
  reps: RepRollup[];
  team: { totals: RepRollup['totals']; repCount: number };
}
...
export function AgentActivityClient({ initial, teamName }: Props) {
  void teamName;
  ...
  const headlineNumber = data.team.totals.all;
  const repCount = data.team.repCount;
  ...
  // rep's side. Routes collapse into one bucket so we don't overweight
  // team-only signal in the headline strip.
    ...
    <div className="space-y-8">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-2">
              {headlineNumber === 0
                ...
                : <>actions {periodCopy(windowDays)} across {repCount} {repCount === 1 ? 'rep' : 'reps'}.</>
              }
            </p>
          </div>
      ...
      <section className="space-y-3">
        <div className="flex items-center gap-3 pb-3 border-b border-border/60">
          ...
          {repCount > 0 && (
            <span className="text-[11px] text-muted-foreground tabular-nums">{repCount}</span>
          )}
          {pending && (
        ...
        {data.reps.length === 0 ? (
          <div className="rounded-xl border border-dashed border-border/70 bg-muted/20 px-5 py-10 text-center">
            ...
            <p className="text-xs text-muted-foreground mt-1">
              Invite a rep and Koala&apos;s work will start landing here.
            </p>
          </div>
        ) : (
          <AnimatePresence mode="wait" initial={false}>
            <StaggerList key={`list-${windowDays}`} className="divide-y divide-border/60">
              {data.reps.map((r) => (
                <StaggerItem key={r.spaceId}>
                  <RepRow row={r} maxAll={data.reps[0]?.totals.all ?? 0} />
                </StaggerItem>
              ))}
            </StaggerList>
          </AnimatePresence>
...
function RepRow({ row, maxAll }: { row: RepRollup; maxAll: number }) {
  const name = row.name ?? row.email ?? 'Unnamed';
  const t = row.totals;
  // Visual weight: a thin horizontal bar inside the row whose length is
  // proportional to total activity vs. the busiest rep in the window.
  ...
  return (
    <div className="group/row flex items-start gap-3 py-3">
      ...
      <div className="flex-1 min-w-0 space-y-1.5">
        ...
        {/* Activity bar — proportional to the busiest rep */}
