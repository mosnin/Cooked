...
export default async function KoalaPage({
  ...
  // `prefill` populates the composer but does NOT auto-send — the rep
  ...
  try {
    const { data: convData } = await supabase
      ...
      .select('*')
      .eq('spaceId', space.id)
      // Rep surface NEVER shows manager-Koala or team chat. These live in
    ...
    if (targetConvId) {
      // Verify the requested conversation belongs to THIS rep space and is
      // a rep conversation BEFORE loading any messages. Without this guard
      // an arbitrary conversationId in the URL (a team conversation, or
      ...
      const { data: convRow } = await supabase
        ...
        .eq('id', targetConvId)
        .maybeSingle();
      const isOwnRepConversation = isRepConversation(
        convRow as { spaceId: string; title: string } | null,
        space.id,
      );

      if (isOwnRepConversation) {
  ...
  // morning when the rep has zero active integrations AND Composio is
