 ...
 * team lead-routing engine.
 *
 * The engine under test is `routeTeamLead(teamId, lead?)` in
 * lib/team-routing.ts. With the BP7d layer added, enabled
 ...
 *
 * Mock shape mirrors tests/lib/team-routing.test.ts — a
 * table-keyed chain mock on `@/lib/supabase` with `mockByTable[name] =
 * { rows, single, error }` overrides.
 *
 * NB on pool-tag narrowing: TeamMembership has no `tags` column
...
/**
 * Seed a canonical "three eligible reps" fixture: u_a, u_b, u_c each
 * with a Space inside team b_1. Additional overrides can swap in
...
async function callRouter(lead?: {
  ...
  return mod.routeTeamLead('b_1', lead);
}
...
// Criteria matching
// ──────────────────────────────────────────────────────────────────────────
describe('routeTeamLead + rules — criteria', () => {
...
// Priority & enabled
// ──────────────────────────────────────────────────────────────────────────
describe('routeTeamLead + rules — ordering & flags', () => {
...
// ──────────────────────────────────────────────────────────────────────────
describe('routeTeamLead + rules — destination fallback', () => {
...
// ──────────────────────────────────────────────────────────────────────────
describe('routeTeamLead + rules — pool method', () => {
  it('pool round-robin over the full rep set (no tag narrowing yet)', async () => {
  ...
  it.skip('pool method + destinationPoolTag narrows pool to tagged agents (TODO: requires TeamMembership.tags column)', async () => {
    // The schema accepts destinationPoolTag but the v1 engine ignores it
    // because there is no tags column on TeamMembership or User yet.
