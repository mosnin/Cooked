...
export async function POST(req: NextRequest) {
  ...
  // ── Rate limit ──────────────────────────────────────────────────────────
  const { allowed } = await checkRateLimit(`manager-import:${team.id}`, 5, 3600);
  if (!allowed) {
  ...
  // Default space: the team owner's space
  const defaultSpaceId = spaceByOwner[team.ownerId] ?? null;
  if (!defaultSpaceId) {
    return NextResponse.json(
  ...
  for (let i = 0; i < dataLines.length; i++) {
    ...
    if (assignTo) {
      ...
      if (!targetUserId) {
        errors.push(`Row ${rowNum}: Member "${assignTo}" not found in team. Using default.`);
      } else {
    ...
    contactsToInsert.push({
      ...
      notes: notes || null,
      type: 'QUALIFICATION',
      tags: ['new-lead', 'imported', 'team-lead'],
