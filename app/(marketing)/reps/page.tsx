/**
 * `/reps` — Koala for the solo rep who's boots-on-the-ground.
 *
 * The story: a rep flying solo spends the day in the field — at the
 ...
 * tabs, and the rep stays in the driver's seat on every send.
 *
 ...
 *
 * Auth-aware like the homepage: signed-in reps bounce to their workspace.
 */
...
import { RepsHero } from '@/components/marketing/reps/reps-hero';
import { FieldSurfaces } from '@/components/marketing/reps/field-surfaces';
import { FeatureRow } from '@/components/marketing/reps/feature-row';
import {
  OnePlace,
  TailorIt,
} from '@/components/marketing/reps/consolidation-tailor';
import { ClosingCTA } from '@/components/marketing/reps/closing-cta';
import {
...
export const metadata = {
  title: 'For solo reps · Koala',
  description:
    'Koala is the extra teammate in the field, reachable on your phone, the web app, or the office desktop. It drafts in your voice, scores the lead, books the demo, and keeps every deal current.',
};

export default async function RepsPage() {
  const { userId } = await auth();
  if (userId) {
    redirect('/auth/redirect?intent=rep');
  }
