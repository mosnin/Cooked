...
/**
 * Rep onboarding — six stages, ~4 minutes.
 *
 * Replaces the one-screen `OnboardingQuick`. The point isn't to "collect more
 * fields" — it's that by the end, Koala already knows the rep's role,
...
type Role = 'solo' | 'team_lead' | 'team_owner';
...
export function OnboardingRep({ defaultName }: Props) {
  ...
  // business name, link the new Space to the new Team, and route to
    ...
    try {
      ...
      if (role === 'team_owner') {
        ...
        if (newTeamId) {
          ...
          // team exists and the user is a manager_owner either way.
        ...
        toast.success("Team created. Koala is ready.");
        router.push('/manager');
        return;
      }
    ...
    <div className="space-y-8">
      ...
      <Section label="What are you running?">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
          ...
          <PickerButton selected={props.role === 'team_owner'} onClick={() => props.onChangeRole('team_owner')}>
            <span className="font-medium">I own the team</span>
          </PickerButton>
        </div>
      </Section>
...
function StageVoice({
  ...
  // Same scenario for both — only tone differs. Rep picks the one that
