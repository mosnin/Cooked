...
export function MembersClient({
  ...
  // One-sentence status read of the team. Loud fact wins. Team name
  // anchors the empty case so the page still reads as "your team."
  const subtitle = (() => {
    if (members.length === 0) return `Nobody on the team at ${teamName} yet.`;
    if (pendingCount > 0) {
              ...
              {filtered.map((m) => {
                const canManage =
                  ...
                  (currentUserRole === 'manager_owner' ||
                    (currentUserRole === 'manager_admin' && m.role === 'rep_member'));
