/**
 * BP6d — tests for the team-template library + publish fan-out.
 *
 ...
 * the publish fan-out: the route reads N TeamMembership rows then
...
type ManagerRole = 'manager_owner' | 'manager_admin' | 'rep_member' | null;
interface AuthState {
  clerkId: string | null;
  dbUserId: string;
...
vi.mock('@/lib/permissions', () => ({
  requireManager: vi.fn(async () => {
    if (!authState.teamRole || authState.teamRole === 'rep_member') {
      throw new Error('Forbidden: manager access required');
    }
    return {
      membership: { id: 'm_1', role: authState.teamRole, userId: authState.dbUserId, teamId: authState.teamId },
      team: { id: authState.teamId, name: 'Test Team', ownerId: authState.dbUserId },
      dbUserId: authState.dbUserId,
    };
  }),
  getManagerMemberContext: vi.fn(async () => {
    if (!authState.teamRole) return null;
    return {
      membership: { id: 'm_1', role: authState.teamRole, userId: authState.dbUserId, teamId: authState.teamId },
      team: { id: authState.teamId, name: 'Test Team', ownerId: authState.dbUserId },
      dbUserId: authState.dbUserId,
...
// A fully-formed TeamTemplate row — used as the `single` payload the
// supabase mock returns from `.single()`/`.maybeSingle()` calls on the
// TeamTemplate table. Version/publish counters default to an initial
...
describe('POST /api/manager/templates', () => {
  ...
  it('403 when caller role is rep_member', async () => {
    authState.teamRole = 'rep_member';
  ...
  it('201 on happy path — inserted row has version=1', async () => {
    // The route inserts and selects the row back. Steer the TeamTemplate
    // `single` payload so the returned body matches what we'd expect the DB
    // to have written.
    mockByTable.TeamTemplate = {
    ...
    const inserted = insertCalls.TeamTemplate ?? [];
...
describe('PATCH /api/manager/templates/[id]', () => {
  ...
  it('404 when row not in caller team', async () => {
    mockByTable.TeamTemplate = { single: null };
  ...
  it('version increments when body changes (1 → 2)', async () => {
    ...
    const updates = updateCalls.TeamTemplate ?? [];
  ...
  it('empty patch body → 400 rather than bumping version', async () => {
    ...
    // not-found branch.
    mockByTable.TeamTemplate = {
      single: makeTemplateRow({ id: 't_1', teamId: 'b_1', version: 1 }),
    };
...
describe('DELETE /api/manager/templates/[id]', () => {
  ...
  it('204 on happy path', async () => {
    mockByTable.TeamTemplate = {
      single: makeTemplateRow({ id: 't_1', teamId: 'b_1' }),
    };
    const res = await invoke('t_1');
    expect(res.status).toBe(204);
    expect(deleteCalls.TeamTemplate ?? 0).toBeGreaterThanOrEqual(1);
  });

  it('404 when row not in team', async () => {
    mockByTable.TeamTemplate = { single: null };
...
describe('POST /api/manager/templates/[id]/publish', () => {
  ...
  it('happy path: pushes to 2 agents, skips 1 with sourceVersion=null; publishedCount=2', async () => {
    ...
    // TeamMembership list — three rep_member rows.
    mockByTable.TeamMembership = {
      rows: [
        { id: 'm_a', userId: 'u_a', teamId: 'b_1', role: 'rep_member' },
        { id: 'm_b', userId: 'u_b', teamId: 'b_1', role: 'rep_member' },
        { id: 'm_c', userId: 'u_c', teamId: 'b_1', role: 'rep_member' },
      ],
    };

    // Space rows — each agent has one space in team b_1.
    mockByTable.Space = {
      rows: [
        { id: 'space_a', ownerId: 'u_a', teamId: 'b_1' },
        { id: 'space_b', ownerId: 'u_b', teamId: 'b_1' },
        { id: 'space_c', ownerId: 'u_c', teamId: 'b_1' },
      ],
    };
    ...
    // TeamTemplate should have been updated with publishedCount=2 (and
    // publishedAt set).
    const tmplUpdates = updateCalls.TeamTemplate ?? [];
  ...
  it('pushes into a fresh agent (no prior MessageTemplate) via INSERT', async () => {
    ...
    mockByTable.TeamMembership = {
      rows: [
        { id: 'm_fresh', userId: 'u_fresh', teamId: 'b_1', role: 'rep_member' },
      ],
    };
    // Agent's space.
    mockByTable.Space = {
      rows: [{ id: 'space_fresh', ownerId: 'u_fresh', teamId: 'b_1' }],
    };
