...
type EditorMode = { kind: 'create' } | { kind: 'edit'; template: TeamTemplate };
...
function publishStatus(t: TeamTemplate): PublishStatus {
...
function formFromTemplate(t: TeamTemplate): EditorForm {
  return {
    name: t.name,
    category: t.category,
    channel: t.channel,
...
export default function TemplatesClient() {
  ...
  const [publishTarget, setPublishTarget] = useState<TeamTemplate | null>(null);
  ...
  const loadTemplates = useCallback(async () => {
    ...
    try {
      ...
      const data = (await res.json()) as TeamTemplate[];
      setTemplates(data);
    } catch (err) {
  ...
  function openEdit(t: TeamTemplate) {
  ...
  async function saveOnly(): Promise<TeamTemplate | null> {
    ...
    const saved = (await res.json()) as TeamTemplate;
    setTemplates((prev) => {
  ...
  async function handleDuplicate(t: TeamTemplate) {
    try {
      ...
      const copy = (await res.json()) as TeamTemplate;
  ...
  const grouped = useMemo(() => {
    const byCat: Record<TemplateCategory, TeamTemplate[]> = {
      'follow-up': [],
      intro: [],
      closing: [],
      'demo-invite': [],
