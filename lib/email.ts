 ...
 * emails Resend rejected — reps were told their messages went out when
...
export interface TeamInvitationEmailParams {
  ...
  teamName: string;
  inviterName: string;
  roleToAssign: 'manager_admin' | 'rep_member';
  token: string;
}

export async function sendTeamInvitation(params: TeamInvitationEmailParams): Promise<void> {
  if (!process.env.RESEND_API_KEY) { logger.warn('[email] RESEND_API_KEY not set — skipping'); return; }
  const { Resend } = await import('resend');
  const resend = new Resend(process.env.RESEND_API_KEY);
  const FROM = getFromAddress();
  ...
  const { toEmail, teamName, inviterName, roleToAssign, token } = params;
  const acceptUrl = `${appUrl}/invite/${token}`;
  const roleLabel = roleToAssign === 'manager_admin' ? 'Team Admin' : 'Rep';
...
<body style="margin:0;padding:0;background:#f9fafb;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif">
  <table width="100%" cellpadding="0" cellspacing="0" style="background:#f9fafb;padding:32px 16px">
    <tr><td align="center">
      <table width="100%" cellpadding="0" cellspacing="0" style="max-width:520px;background:#ffffff;border-radius:12px;border:1px solid #e5e7eb;overflow:hidden">
        <tr><td style="background:#0f172a;padding:20px 28px">
          <p style="margin:0;color:#94a3b8;font-size:12px;font-weight:500;text-transform:uppercase;letter-spacing:.05em">Team Invitation</p>
        ...
        <tr><td style="padding:24px 28px">
          <p style="margin:0;font-size:15px;color:#111827;line-height:1.6">
            <strong>${esc(inviterName)}</strong> has invited you to join <strong>${esc(teamName)}</strong> as a <strong>${esc(roleLabel)}</strong> on Koala.
          </p>
          <p style="margin:12px 0 0;font-size:13px;color:#6b7280;line-height:1.5">
            You'll keep your own workspace, leads, and pipeline. This just adds you to the team network.
          </p>
          <table width="100%" cellpadding="0" cellspacing="0" style="margin-top:24px">
            <tr><td>
        ...
        <tr><td style="padding:16px 28px;border-top:1px solid #f1f5f9">
          <p style="margin:0;font-size:11px;color:#9ca3af">You received this because someone invited ${esc(safeEmail)} to a Koala team. If this was a mistake, you can ignore this email.</p>
        </td></tr>
      </table>
    </td></tr>
  </table>
  ...
  try {
    ...
    if (result.error) {
      logger.error('[email] team invitation: Resend API error', { to: redactEmail(toEmail), resendError: result.error });
    } else {
      logger.info('[email] team invitation sent', { to: redactEmail(toEmail), messageId: result.data?.id });
    }
  } catch (err) {
    logger.error('[email] team invitation failed', { to: redactEmail(toEmail) }, err);
  }
}
