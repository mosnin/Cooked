...
export async function GET(req: NextRequest) {
  ...
  const [settingsRes, ownerRes] = await Promise.all([
    supabase
      ...
      .select(
        ...
        'bio, socialLinks, businessName, repPhotoUrl, privacyPolicyHtml,' +
  ...
  return NextResponse.json({
    settings: {
      ...
      repPhotoUrl: settings?.repPhotoUrl ?? '',
...
export async function PATCH(req: NextRequest) {
  ...
  const repPhotoUrl = typeof body.repPhotoUrl === 'string' ? body.repPhotoUrl.slice(0, 500)  : undefined;
  ...
  if (body.teamId && typeof body.teamId === 'string') {
    // SECURITY: only allow associating with a team the owner is actually a
    // member of. teamId drives credit-pool routing (lib/billing/account.ts);
    ...
    const { data: membership } = await supabase
      ...
      .maybeSingle();
    if (membership) updateFields.teamId = body.teamId;
  }
  ...
  if (repPhotoUrl !== undefined) settingsPayload.repPhotoUrl = repPhotoUrl;
