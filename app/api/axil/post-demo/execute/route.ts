...
interface ResultOut {
  ...
  summary: string;
  error?: string;
  /** Rep-voice verb for the post-execute toast. Only set on success. */
  doneVerb?: string;
}
...
export async function POST(req: NextRequest) {
  ...
  // integration tools must reference a toolkit the rep has actually
  ...
  for (const p of body.proposals) {
    ...
    if (nativeAllow.has(tool)) {
      ...
      // Integration proposal — accept only if (1) the rep still has
 ...
 * (which the rep approved) + the response (which gives us the
 * Google event id). Never throws — the rep's calendar already has
