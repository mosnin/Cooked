...
import { notificationForNewTeamLead } from '@/lib/notification-voice';
...
import { routeTeamLead } from '@/lib/team-routing';
...
/**
 * Team-specific intake schema.
 * Same as the regular application schema but uses `teamId` instead of `slug`.
 */
const teamApplicationSchema = publicApplicationSchema
  .omit({ slug: true })
  .extend({
...
/**
 * Resolve the correct form config for a team submission.
 *
 * Fallback chain:
 *   1. Team dual config: [teamRentalFormConfig | teamBuyerFormConfig]
 *   2. Team legacy: teamFormConfig (if leadType matches)
 ...
 */
async function fetchTeamFormConfig(
  teamId: string,
  spaceId: string,
  leadType: 'rental' | 'buyer',
): Promise<IntakeFormConfig | null> {
  try {
    // First try team-level configs directly
    ...
    if (team) {
      ...
      const dualRaw = leadType === 'buyer'
        ? team.teamBuyerFormConfig
        : team.teamRentalFormConfig;
      if (dualRaw) {
    ...
    logger.warn('[apply/team] form config fetch failed', { teamId, spaceId, leadType }, err);
  }
...
export async function POST(req: NextRequest) {
  ...
  const { allowed } = await checkRateLimit(`apply-team:rl:${ip}`, 10, 3600);
  if (!allowed) {
  ...
  try {
    ...
    logger.warn('[apply/team] invalid JSON body', undefined, error);
    return NextResponse.json({ error: 'Invalid request body' }, { status: 400 });
  }

  // Extract teamId and leadType from raw body before validation
  const rawBody = typeof requestBody === 'object' && requestBody !== null
    ? (requestBody as Record<string, unknown>)
    : {};
  const rawTeamId = rawBody.teamId;
  ...
  try {
    ...
    if (!team || team.status !== 'active') {
      // Return a generic error for both invalid and not-found teams
      // to prevent ID enumeration attacks
      logger.warn('[apply/team] invalid or inactive team', { teamId: rawTeamId });
      return NextResponse.json({ error: 'Unable to process application. Please check the link and try again.' }, { status: 422 });
    }

    // ── Find the team-linked Space owned by the manager owner ──────────
    let space: { id: string; slug: string; name: string; ownerId: string; teamId: string | null } | null = null;

    const { data: linkedSpace, error: spaceError } = await supabase
      .from('Space')
      .select('id, slug, name, ownerId, teamId')
    ...
    if (!space) {
      ...
      if ((ownerSpaces ?? []).length === 1 && fallbackSpace) {
        space = fallbackSpace;
        logger.warn('[apply/team] using legacy owner-only space fallback', {
          teamId: team.id,
    ...
    if (!space) {
      logger.error('[apply/team] manager owner has no space', {
        teamId: team.id,
        ownerId: team.ownerId,
      });
      return NextResponse.json({ error: 'Team configuration error' }, { status: 500 });
    }

    // ── Fetch the correct form config based on leadType ────────────────────
    let formConfig: IntakeFormConfig | null = null;
    try {
      formConfig = await fetchTeamFormConfig(team.id, space.id, resolvedLeadType);
      if (formConfig) {
        formConfig = formConfigSchema.parse(formConfig);
      }
    } catch (err) {
      logger.warn('[apply/team] form config invalid or fetch failed, falling back to legacy', {
        teamId: team.id,
        spaceId: space.id,
    ...
    if (formConfig) {
      try {
        ...
        logger.warn('[apply/team] scoring model fetch failed (non-fatal, will use legacy scoring)', {
          spaceId: space.id,
          teamId: team.id,
          leadType: resolvedLeadType,
        }, err);
    ...
    if (formConfig) {
      ...
      logger.debug('[apply/team] using dynamic form config', {
        teamId: team.id,
        spaceId: space.id,
        leadType: resolvedLeadType,
      ...
      if (!parsed.success) {
        logger.warn('[apply/team] dynamic validation failed', { issues: parsed.error.issues });
      ...
      const parsed = teamApplicationSchema.safeParse(requestBody);
      if (!parsed.success) {
        logger.warn('[apply/team] validation failed', { issues: parsed.error.issues });
        return NextResponse.json({ error: 'Invalid submission data' }, { status: 400 });
      }
    ...
    const idempotencyKey = `apply-team:idempotency:${fingerprint}`;
    ...
    try {
      ...
      logger.warn('[apply/team] idempotency lock unavailable; using DB fallback', {
        spaceId: space.id,
      }, error);
    }
    ...
    if (!idempotencyLockAcquired) {
      logger.info('[apply/team] proceeding without distributed lock', {
        spaceId: space.id,
        teamId: team.id,
        fingerprint,
      });
    ...
    try {
      ...
      logger.warn('[apply/team] failed to fetch space settings', { spaceId: space.id }, err);
    }
    ...
    // owner's space (current behaviour). `routeTeamLead` never throws
    ...
    const routing = await routeTeamLead(team.id, {
      leadType: contactLeadType,
    ...
    if (routing) {
      logger.info('[apply/team] auto-assigned to agent', {
        teamId: team.id,
    ...
    logger.info('[apply/team] submission persisted', {
    ...
    try {
      ...
      const { error: scoreUpdateError } = await supabase
        ...
        .eq('id', contact.id);
      if (scoreUpdateError) {
        logger.error('[apply/team] scoring update failed', {
          contactId: contact.id,
        }, scoreUpdateError);
      } else {
        logger.info('[apply/team] scoring persisted', {
          contactId: contact.id,
          scoringStatus: scoring.scoringStatus,
          scoreLabel: scoring.scoreLabel,
        });
      ...
      logger.error('[apply/team] scoring failed', { contactId: contact.id }, error);
      try {
        await supabase
          .from('Contact')
          .update({
        ...
        logger.error('[apply/team] fallback scoring state failed', {
          contactId: contact.id,
        }, fallbackErr);
      }
    }

    // Send team dashboard notification + applicant confirmation email in parallel
    const businessName = spaceBusinessName || team.name || space.name;

    const managerLeadCopy = notificationForNewTeamLead(contactName, {
      phone: contactPhone,
      email: contactEmail,
    });
    const managerNotification = notifyManager({
      ...
      logger.error('[apply/team] team notification failed', { contactId: contact.id }, err);
    });
    ...
    const applicantConfirmation = contactEmail
      ? sendApplicationConfirmation({
          ...
          logger.error('[apply/team] applicant confirmation email failed', { contactId: contact.id }, confirmErr);
        })
      : Promise.resolve();

    await Promise.all([managerNotification, applicantConfirmation]);
    logger.debug('[apply/team] notifications dispatched', { contactId: contact.id });
    ...
    logger.error('[apply/team] unhandled submission failure', {
      teamId: rawTeamId,
    }, error);
