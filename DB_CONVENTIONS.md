...
Demo, DemoPropertyProfile, DemoAvailabilityOverride, DemoWaitlist, DemoFeedback
Conversation, Message, DocumentEmbedding
Team, TeamMembership, Invitation
...
- Roles: lowercase — `'user'`, `'admin'`, `'manager_owner'`, `'manager_admin'`, `'rep_member'`
- Team status: lowercase — `'active'`, `'suspended'`
...
- Use `ON DELETE RESTRICT` for ownership that must be reassigned first (e.g., `Team.ownerId`)
...
- Tables without `spaceId`: `User`, `Team`, `TeamMembership`, `Invitation`, `AuditLog`
...
| `create_team_with_owner` | Create team + owner membership | Team creation endpoint |
