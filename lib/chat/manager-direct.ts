/**
 * Manager direct path — the in-process fast lane for the team chat.
 *
 * The manager chat used to send EVERY turn to Modal (cold start, the same
 * fragility the rep chat was moved off of). Most manager questions are
 * read-only Q&A — "how's the team doing?", "how many leads are waiting?",
 * "what's our pipeline?" — which need no tools, just the team's current
 ...
 *
 * Mirrors `lib/chat/direct-stream.ts` (the rep direct path): same SSE
 * shape, same persistence + usage recording, just a team-scoped context
...
team using the live snapshot below: team size, pipeline, leads waiting,
...
and point them at the relevant page (Reps, Deals, Forecast).
`.trim();
...
/** Aggregate the team's current numbers into a compact context block the
 *  direct LLM can answer from. Cheap, parallel counts across member spaces. */
async function buildTeamSnapshot(team: { id: string; name: string; ownerId: string }): Promise<string> {
  ...
  const repCount = members.filter((m) => m.role === 'rep_member').length;
  if (spaceIds.length === 0) {
    return `Team snapshot (${team.name}):\n- Reps: ${repCount}\n- No member workspaces with data yet.`;
  }
  ...
  return [
    `Team snapshot (${team.name}), as of now:`,
    `- Reps on the team: ${repCount}`,
...
/**
 * Stream a manager Q&A turn in-process. Same SSE protocol as the rep direct
...
export function streamManagerDirectTurn(input: ManagerDirectInput): Response {
  ...
  const stream = new ReadableStream<Uint8Array>({
    async start(controller) {
      ...
      try {
        const snapshot = await buildTeamSnapshot(input.team);
        ...
        if (result.text.trim()) {
          ...
          try {
            await saveManagerAssistantMessage({ teamId: input.team.id, conversationId: input.conversationId, blocks });
          } catch (err) {
            logger.warn('[manager-direct] save assistant message failed', { teamId: input.team.id }, err);
          }
        }
        ...
        if (!aborted) {
          logger.error('[manager-direct] crashed', { teamId: input.team.id }, err);
