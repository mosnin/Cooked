...
export default function ManagerFormBuilderPage() {
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      toast.success(`Team ${activeLeadType === 'rental' ? 'rental' : 'buyer'} form saved.`);
    } catch (err) {
  ...
  const handleReset = useCallback(async () => {
    ...
    try {
      ...
      toast.success(`Team ${label} form reset to the standard Koala default.`);
    } catch (err) {
      toast.error(err instanceof Error ? err.message : 'Something went wrong.');
    }
  }, [activeLeadType]);
    ...
    <div className="space-y-6 max-w-3xl mx-auto pb-56 md:pb-24">
        ...
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
          <header className="space-y-1.5">
            ...
            <p className={BODY_MUTED}>
              {hasChanges
                ? 'Unsaved changes. Save to update your team standard.'
                : hasSavedConfig
                  ? 'Saved as your team standard.'
        ...
        <div className="space-y-1.5 border-b border-border/60 pb-4">
          ...
          {!hasSavedConfig && (
            <p className="text-xs text-muted-foreground">
              No custom team {activeLeadType === 'rental' ? 'rental' : 'buyer'} form yet. Customize the fields below and save to create your team standard.
            </p>
          )}
        </div>
      </div>
      ...
      <div className="space-y-2 border-b border-border/60 pb-6">
        ...
        {!eitherHasSavedConfig && !anyHasChanges && (
          <p className="text-xs text-muted-foreground">
            Save a custom team form first before pushing to members.
          </p>
        )}
      </div>
      ...
      <AlertDialog open={resetConfirmOpen} onOpenChange={setResetConfirmOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This restores the standard Koala default and removes your custom team {activeLeadType === 'rental' ? 'rental' : 'buyer'} form.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
      ...
      <AlertDialog open={pushConfirmOpen} onOpenChange={setPushConfirmOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This overrides both the rental and buyer form settings for {memberLabel} with your team forms. Their form source switches to the team standard.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
