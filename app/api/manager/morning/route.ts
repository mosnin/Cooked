 ...
 *   2. unassigned-leads — team-tagged Contacts sitting in the queue.
 *   3. overloaded       — a rep whose open work (active deals + open
 *                         leads) exceeds 15 — the canonical team_health
 *                         threshold from agent/tools/manager/team.py.
 *   4. underused        — a rep with open work below 3 who's still
 *                         active in the last 30 days (canonical team_health
 *                         underused signal). Newly-invited dormant members
 *                         don't surface — only currently-active reps
 *                         going quiet.
 ...
 *     stats: { reps, activeDeals, gciMtd } }
 *
 * `suggestedPrompt` is non-null on branches 1-4 (the "want me to ..."
 * affordance); the component routes the manager into /manager/koala with the
...
export interface ManagerMorningResponse {
  ...
  headline: string;
  /**
   * Text to prefill into /manager/koala when the manager taps the headline.
...
// agent/tools/manager/pipeline.py:42 so the row pill and Koala answer the
...
// the reps page and team_health use to define "responding".
...
/** First name only — manager speaks about reps the way they call them. */
...
interface RepLoad {
  ...
  openLoad: number;
  /** True when the rep has any outbound activity in the last 30 days. */
...
export async function GET() {
  ...
  const memberRows: MemberRow[] = members
    .map((m) => ({
      userId: m.userId,
      name: m.User?.name ?? m.User?.email ?? 'Rep',
      spaceId: m.Space?.id ?? '',
    }))
  ...
  // Six queries; nothing fancier than what /manager/reps already does.
  // Each is team-scoped through `spaceId IN (...)` or
  // `teamId = ...` so RLS doesn't leak across teams even if
  ...
  const [
    ...
    // Every active deal — just spaceId for the per-rep active-deals count
    ...
    // Open leads per rep — Contact WHERE type='QUALIFICATION'. This is
    ...
    // Unassigned team leads — same predicates as
    // app/manager/leads/page.tsx and find_unassigned_leads:
    // teamId match, tag includes 'team-lead', NOT 'assigned'.
    supabase
      .from('Contact')
      .select('id, tags')
      .eq('teamId', team.id)
  ...
  // ── Stuck deals: rank by (days × value), pick the top rep ───────────
  ...
  for (const [spaceId, rows] of stuckBySpace) {
    ...
    // applied per rep instead of per deal. Floor of $1 on value so a
  ...
  // ── Per-rep open load (active deals + open leads) ───────────────────
  ...
  const loads: RepLoad[] = memberRows.map((m) => {
  ...
  // Overloaded = highest open_load AND open_load > 15. Single-rep team
  // never trips this — the manager can't redistribute to themselves.
  let overloaded: RepLoad | null = null;
  if (loads.length >= 2) {
  ...
  // copy: we don't call out a rep who's already on vacation or hasn't
  // started. Same guard as team_health._load_signal.
  let underused: RepLoad | null = null;
  if (loads.length >= 2) {
  ...
  // 2. Unassigned leads.
    ...
    headline = `${n} ${leadWord} sitting unassigned — want me to route ${n === 1 ? 'it' : 'them'} to your fewest-loaded rep?`;
    suggestedPrompt =
      n === 1
        ? 'Route the unassigned lead to the rep with the lightest load.'
        : `Route the ${n} unassigned leads to the rep with the lightest load.`;
  }
  // 3. Overloaded rep.
  else if (overloaded) {
    const fn = firstName(overloaded.name);
    // Fact framing — never "Bob is slow," always "Bob's at 18 active vs
    // threshold 15." Mirrors the row-pill voice from /manager/reps.
