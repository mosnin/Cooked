 ...
 * context-sensitive: the rep's name, the workspace, today's date,
...
/**
 * Personalized prompt — same baseline plus a snapshot block (rep name,
...
export async function buildPersonalizedSystemPrompt(
  ...
  try {
    ...
    if (opts.integrations && opts.integrations.unavailableToolkits.length > 0) {
      ...
      // rep their integrations are gone.
      snapshotBlock = [
        snapshotBlock,
        `Note: the rep's ${names} connection${opts.integrations.unavailableToolkits.length === 1 ? ' is' : 's are'} temporarily unreachable this turn. If asked, say so — do NOT claim ${opts.integrations.unavailableToolkits.length === 1 ? 'it is' : 'they are'} disconnected or missing.`,
      ]
        .filter(Boolean)
        .join('\n');
    }
...
function composePrompt(ctx: ToolContext, opts: BuildOptions, snapshotBlock: string): string {
  ...
  lines.push(
    ...
    `You have up to 15 tool turns per reply. Use them. When a task requires a chain of lookups — find a person → read their activity → locate their deal → draft a follow-up — execute every step in sequence WITHOUT stopping to ask the rep for permission or progress updates between steps. Complete the full task, THEN surface the result.`,
    ``,
    `Concretely:`,
    ...
    `Call \`planner\` FIRST — before any other tool — when a task requires 3 or more tool calls OR coordinates across multiple people, deals, or calendar events. The plan is shown to the rep before execution; after that, execute every announced step in order.`,
    ``,
    `When to plan:`,
    ...
    `- After calling \`delegate_task\`, tell the rep in one sentence that you've kicked it off. The live card shows the rest; don't narrate its steps.`,
    ...
    `- Mutating tools (send_email, create_deal, etc.) always require rep approval. Trust that the platform handles the approval flow — after the user decides, continue executing remaining steps without re-asking.`,
