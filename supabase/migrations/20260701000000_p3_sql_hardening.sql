...
--
-- 3. Partial UNIQUE index on stripeSubscriptionId (Space + Team). A Stripe
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'Team_defaultAgentRate_range') THEN
    ALTER TABLE "Team" ADD CONSTRAINT "Team_defaultAgentRate_range"
      CHECK ("defaultAgentRate" IS NULL OR ("defaultAgentRate" >= 0 AND "defaultAgentRate" <= 100));
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'Team_defaultManagerRate_range') THEN
    ALTER TABLE "Team" ADD CONSTRAINT "Team_defaultManagerRate_range"
...
CREATE UNIQUE INDEX IF NOT EXISTS uq_team_stripe_subscription
