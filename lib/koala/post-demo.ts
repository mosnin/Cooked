 ...
 * ask the model to propose 2-5 actions the rep would have taken
 * manually. The model does NOT execute the tools — it returns intent
 * only. The rep approves the batch in one tap; the route then runs
 ...
 *    rep who wants to review before anything happens.
 ...
 * truncate to 5. Either failure mode is the model's fault; the rep
 ...
 *  these are administrative or aggregate queries the rep would never
...
/**
 * A Composio tool the rep's connected accounts expose. We pull this
 ...
 * for the done-verb resolver, NOT shown to the rep in the proposal
...
export interface IntegrationToolSpec {
  ...
  slug: string;
  /** Rep-readable description. May be Composio's text, may be pre-processed. */
  description: string;
...
export interface ProposedAction {
  ...
  humanSummary?: string;
  /** Whether this verb mutates state (i.e. needs the rep's yes). */
  mutates: boolean;
  /**
 ...
 *
 *  When the rep has connected apps (Gmail, Google Calendar, ...),
...
export function buildSystemPrompt(
  ...
  const lines: string[] = [
    'Turn a rep\'s post-demo debrief into 2-5 proposed actions. Return intent only — never execute.',
    '',
    'Allowed tools:',
    native,
  ];
  ...
  if (integrationTools.length > 0) {
    ...
    lines.push(
      '',
      'Connected-app tools (the rep has authorized these):',
      integ,
      '',
      'When the rep\'s intent is to actually send, schedule, or post — not draft — prefer the connected-app tool.',
      ...
      'If the rep only wants to see what they\'d say (a draft), use the draft_* tools instead.',
    );
  }
 ...
 *  the in-process registry; integration tools come from the rep's
...
/** Rep-voice summary for an integration proposal. The toolkit slug
...
function integrationProposalSummary(
  ...
  const verb = repVerbForToolkit(toolkit);
...
/**
 * Resolve the Composio tools the rep's post-demo orchestrator should
 ...
 * native catalog and the rep never sees the seam.
 *
 ...
 *  `GMAIL_SEND_EMAIL` → `gmail`. Cross-checks against the rep's
...
export function formatHumanSummary(
  ...
  // Integration proposals come from connected apps. The rep doesn't
  ...
  if (proposal.integrationToolkit) {
    const verb = repVerbForToolkit(proposal.integrationToolkit);
...
/**
 * Rep-voice verb for the proposal stack. The rep doesn't know
 ...
 */
export function repVerbForToolkit(toolkit: string): string {
  switch (toolkit) {
 ...
 * the rep reads this on the way back to their car. Falls back to a
