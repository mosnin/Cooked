...
- **Docs: Team feature spec** — Landed `docs/TEAM_SPEC.md` in four chunks covering concepts + permissions + data model, feature map for BP1–BP7 and linear steps 1–3, and the full route inventory + audit-logging notes (`b680f91`, `02e2b5b`, `14cedb5`, `491a103`).
...
Manager-side features delivered in seven phases. See `docs/TEAM_SPEC.md` for the full feature map.

- **BP1 — agent offboarding** — Atomic RPC transfers contacts/deals/demos when removing a rep; dialog wired + vitest coverage (`6db1e83`, `345e46d`).
...
- **BP6 — template versioning** — Team templates schema + magic-Note extraction, GET/POST rewrite, `[id]` routes + tests, UI rewrite onto the new API, and security/correctness fixes (`46612ab`, `3246091`, `3e9196b`, `2571397`, `e274236`).
- **BP7 — lead routing** — Auto-routing schema on `Team`, routing engine + settings wiring + tests, plus a no-mutate fix on Supabase responses (`d3176d4`, `511adb5`, `380fd83`).

### Linear steps 1–3

- **Step 1 — Rep reviews** — New rep-side "My reviews" page surfacing the manager's review requests (`b524072`).
...
- **Enhancement: Dashboard redesign** — Aligned manager dashboard with rep dashboard design patterns:
  ...
  - Max-width 1120px constraint matching rep dashboard
- **Feature: Manager onboarding checklist** — 4-step guided setup (invite reps, configure settings, review performance, export report) with progress bar and localStorage-persisted dismissal.
...
- **Feature: Search on reps page** — Client-side search input filtering reps by name or email.
