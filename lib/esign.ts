/**
 * E-signature via Composio — the rep's OWN connected DocuSign account.
 *
 ...
 * rep connects it through the same OAuth flow as Gmail/Slack; Composio
 * owns the OAuth app and holds the tokens. We send and read envelopes on
 * the rep's behalf by executing Composio DocuSign actions, scoped to
 * the rep's Clerk userId (the Composio "entity").
 *
...
/** True when the rep has an active DocuSign connection on Composio. */
...
/**
 * Send a stored document out for signature on the rep's DocuSign account.
...
export async function sendForSignature(
  ...
  // 4. Fire the Composio CREATE_ENVELOPE action on the rep's account.
