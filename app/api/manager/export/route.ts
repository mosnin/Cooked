...
export async function GET() {
  ...
  const rows = members.map((m) => {
    ...
    const role = m.role === 'manager_owner' ? 'Owner' : m.role === 'manager_admin' ? 'Admin' : 'Rep';
  ...
  return new NextResponse(csv, {
    headers: {
      ...
      'Content-Disposition': `attachment; filename="${team.name.replace(/[^a-zA-Z0-9]/g, '_')}_team_export_${new Date().toISOString().split('T')[0]}.csv"`,
    },
  });
}
