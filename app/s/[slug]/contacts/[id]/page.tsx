...
export default async function ClientDetailPage({
  ...
  // Skipping this lets any logged-in rep see any other rep's contacts.
  ...
  let contact: (Contact & { dealContacts: { deal: { id: string; title: string; address: string | null; value: number | null; status: string; priority: string; stage: { name: string; color: string } } }[]; demos: { id: string; startsAt: string; endsAt: string; status: string; propertyAddress: string | null }[] }) | null = null;
  ...
  try {
    ...
    if (contactError && contactError.code === 'PGRST116') {
      ...
      const { data: demoRows } = await supabase.from('Demo').select('id, guestName, startsAt, endsAt, status, propertyAddress').eq('contactId', id).eq('spaceId', space.id).order('startsAt', { ascending: false }).limit(10);
  ...
  // E-signature — let the rep send a document from this person's linked
  ...
  return (
    <ContactDetailFrame className="max-w-4xl mx-auto space-y-8 pb-12">
      {/* Headline — name is the page. The next four lines tell the rep
      ...
      {/* Action pills — same vocabulary as the morning home's compose
          actions. State picks them; the rep doesn't. Tap a verb pill
          and the inline draft surface opens beneath; the rep reviews,
      ...
      {/* Score — open when scored, since "why is this person hot" is often
          the rep's next question. Closed when no score yet. */}
      ...
      {/* Send a document to sign — pull a document from this person's deals,
          send it to them on the rep's connected DocuSign, watch status.
      ...
      {/* Application — open when received/null (needs review), closed once
          processed. The rep's eye goes where the work is. */}
      {app && (
      ...
      {/* Pipeline — closed by default; not where the rep's eye goes. */}
      <details className="group border-t border-border/60 pt-4">
        <summary className="cursor-pointer list-none flex items-center justify-between text-sm font-semibold text-foreground hover:text-foreground/80 transition-colors">
      ...
      {/* Follow-up + lifecycle — at the bottom because they're edits-on-this-
          record, not the rep's daily task. */}
      <details className="group border-t border-border/60 pt-4">
        <summary className="cursor-pointer list-none flex items-center justify-between text-sm font-semibold text-foreground hover:text-foreground/80 transition-colors">
...
function derivePersonState(
  ...
  // past today reads as "archived for action-pill purposes" — the rep
 ...
 * thing the rep already cared enough to write down.
 */
