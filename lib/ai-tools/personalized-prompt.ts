/**
 * Per-rep context that gets folded into the agent's system prompt at
 ...
 * gives the model the rep's name + the loudest pipeline facts before
 * it picks up a tool. It saves tool calls AND sounds like the agent knows
 * the rep.
 *
 * What it pulls:
 *   - Rep's first name (Clerk → User table)
 ...
 *     them by rep verb without naming SDK tool slugs)
 *
 * What it does NOT pull:
 *   - Full timelines or activity dumps. The agent has tools for that.
 *   - PII beyond first name. The model shouldn't be reciting the rep's
...
export interface PersonalizedSnapshot {
  /** Rep's first name. Null if we couldn't resolve. */
  ...
  /** Pending AgentDraft rows the rep needs to decide on. */
