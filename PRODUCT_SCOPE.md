...
Koala is an **agentic operating system for U.S. real estate agents and teams.**

A rep's book of business — contacts, leads, deals, demos, properties, applications — is the workspace. Koala is an autonomous AI agent that works *inside* that workspace on the rep's behalf: it qualifies inbound leads, drafts and sends follow-up, schedules demos, advances deals, produces marketing content, and surfaces what needs attention — taking sign-off only where a human decision is genuinely required.
...
- **On request** — the rep talks to Koala in chat; it does the job and reports back.
- **On its own** — workspace events (new lead, application submitted, demo completed, deal stage change, inbound message) and scheduled sweeps wake Koala to act in near real-time, without being asked.
...
- **Solo and independent reps** — Koala runs the lead pipeline end to end.
- **Teams** — manager owners and admins oversee a team of agents: lead routing, commissions, deal review, performance. The team tier is *part of one product*, not a separate one — an operating system for real estate spans the individual agent and the firm they belong to.
...
- **Retention signal**: applications flowing in, and the rep returning to act on what Koala surfaced

"Protect the wedge" means: keep the **first-run experience** fast and unsprawled — sign-up to live intake link stays minimal. It does **not** mean the product stops at renter leads. Depth elsewhere is welcome; friction on a new rep's path to first value is not.
...
The test, when unsure: *does this make Koala more of an operator, or more of a tool the rep operates?* Operator wins.
...
- **The agent earns trust**: Koala takes real actions — scored leads, drafted follow-up, booked demos — and the rep sees and approves them
- **Retention**: the rep returns to act on what Koala surfaced, and lets it do more over time
- **Team**: managers run team oversight — routing, commissions, review — through Koala rather than spreadsheets
