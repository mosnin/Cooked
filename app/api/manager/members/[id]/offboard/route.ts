 ...
 * and offboard them from the team.
 *
 ...
 * different, active user in the same team.
 *
...
export async function POST(req: NextRequest, { params }: Params) {
  ...
  // Only the team owner can offboard members.
  ...
  // Load the target membership, scoped to this team.
  ...
  // Load the destination membership, scoped to this team.
  ...
  if (target.role === 'manager_owner') {
    return NextResponse.json({ error: 'Cannot offboard the team owner' }, { status: 403 });
  }
  ...
  const { data: rpcData, error: rpcError } = await supabase.rpc('offboard_team_member', {
