...
export default async function ManagerBillingPage() {
  ...
  // The team's OWN Stripe identity — written by the team-scoped
  // checkout and kept current by the webhook. When a team subscription
  // exists, it is the billing entity for this page; the owner's personal Space
  // is only a legacy fallback (teams subscribed before team billing).
  const { data: teamStripe } = await supabase
    .from('Team')
  ...
  // If user has no personal space, find the team owner's space
  ...
  const usingTeamEntity = Boolean(teamStripe?.stripeSubscriptionId);

  if (!spaceRow && !usingTeamEntity) {
    return (
  ...
  if (!isManagerOwner) {
    return (
      <div className="space-y-6 max-w-3xl mx-auto pb-12">
        <BillingHeader status="The team owner manages billing." />
        <div className="rounded-xl border border-dashed border-border/70 bg-muted/20 px-5 py-10 text-center">
          <p className="text-sm text-foreground">Billing lives with the owner.</p>
          <p className="text-xs text-muted-foreground mt-1">
            Ask the team owner to make changes to the subscription.
          </p>
        </div>
      </div>
    );
  ...
  // Billing entity: the Team's subscription when it has one, else the
  // legacy owner-space subscription. The previous version of this page ONLY
  // read the owner's personal Space, so a team-scoped subscription showed
  // the wrong status/invoices/card here.
  const billingSubscriptionId = usingTeamEntity
    ? (teamStripe!.stripeSubscriptionId as string)
    : ((spaceRow?.stripeSubscriptionId as string | null) ?? null);
  const billingCustomerId = usingTeamEntity
    ? ((teamStripe!.stripeCustomerId as string | null) ?? null)
    : ((spaceRow?.stripeCustomerId as string | null) ?? null);

  const slug = (spaceRow?.slug as string | undefined) ?? '';
  let subscriptionStatus: 'active' | 'trialing' | 'past_due' | 'canceled' | 'inactive' = usingTeamEntity
    ? mapDbStatus(teamStripe?.stripeSubscriptionStatus)
  ...
  return (
    <div className="space-y-6 max-w-3xl mx-auto pb-12">
      ...
      <BillingPage
        slug={slug}
        plan={(((ctx.team as { plan?: string }).plan) ?? 'free') as PlanId}
        subscriptionStatus={subscriptionStatus}
        currentPeriodEnd={currentPeriodEnd}
        cardLast4={cardLast4}
        cardBrand={cardBrand}
        invoices={invoices}
        // Manage/cancel must act on the team's Stripe identity when the
        // team holds the subscription — the default routes act on a Space
        // the caller owns, which is the wrong entity for a team sub.
        endpoints={
          usingTeamEntity
      ...
      />
      {/* Credits are keyed to the runtime space; a team-entity manager
