 ...
 * rep "what Koala did overnight, ready for your approve" — the
 * single highest-value card type for a working rep whose agent
 * actually ran.
 *
...
export const draftsSource: SignalGatherer = {
  ...
  // shows the rep "Koala did this overnight, ready for your approve."
  ...
  async gather(spaceId: string): Promise<Signal[]> {
    ...
    for (const draft of data as unknown as DraftRow[]) {
      ...
      // working state for the agent, not actionable for the rep's
      ...
      signals.push({
        ...
        // the FocusCard's job, where the rep has the editing UI ready.
        draftedAction: {
          kind: 'open',
