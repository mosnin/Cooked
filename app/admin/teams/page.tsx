...
export default async function AdminTeamsPage() {
    ...
    return (
      <div className="flex min-h-[50vh] items-center justify-center">
        <div className="text-center space-y-2 p-8">
          <p className="text-sm text-muted-foreground">Couldn&apos;t load teams.</p>
          <a href="/admin/teams" className="text-xs text-primary hover:underline">Retry</a>
        </div>
      </div>
    );
  }

  const allIds = (teams ?? []).map((b) => b.id);
  const { data: memberships } = allIds.length > 0
    ? await supabase.from('TeamMembership').select('teamId').in('teamId', allIds)
    : { data: [] };
  ...
  const active    = (teams ?? []).filter((b) => b.status === 'active').length;
  const suspended = (teams ?? []).filter((b) => b.status === 'suspended').length;
    ...
    <div className="space-y-8 pb-12">
      <header className="space-y-1.5">
        ...
        <p className="text-sm text-muted-foreground">
          {teams?.length ?? 0} total · {active} active · {suspended} suspended.
        </p>
      </header>
