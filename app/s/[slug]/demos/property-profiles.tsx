...
export function PropertyProfiles({ slug, profiles: initialProfiles, onUpdate }: PropertyProfilesProps) {
  ...
  async function handleSave() {
    ...
    try {
      if (editingId) {
        const res = await fetch(`/api/demos/properties/${editingId}`, {
          method: 'PATCH',
        ...
        const res = await fetch('/api/demos/properties', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            slug,
  ...
  async function handleDelete(id: string) {
    setDeletingId(id);
    try {
      const res = await fetch(`/api/demos/properties/${id}`, { method: 'DELETE' });
      if (res.ok) {
  ...
  async function toggleActive(id: string, isActive: boolean) {
    setTogglingId(id);
    try {
      const res = await fetch(`/api/demos/properties/${id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-0.5">
            Each property can have its own demo hours, duration, and booking link.
          </p>
        </div>
        {!showForm && (
      ...
      {profiles.length === 0 && !showForm ? (
        <div className="text-center py-10 text-muted-foreground space-y-2">
          ...
          <p className="text-xs">Add properties to give each listing its own demo schedule and booking link.</p>
        </div>
      ) : (
