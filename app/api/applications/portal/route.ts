...
export async function GET(req: NextRequest) {
  ...
  // Mark unread rep messages as read
  if (messages?.some((m: { senderType: string; readAt: string | null }) => m.senderType === 'rep' && !m.readAt)) {
    const unreadIds = messages
      .filter((m: { senderType: string; readAt: string | null }) => m.senderType === 'rep' && !m.readAt)
