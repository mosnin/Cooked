...
export const metadata: Metadata = { title: 'Rep Detail — Teams' };

type Params = { params: Promise<{ userId: string }> };

export default async function RepDrilldownPage({ params }: Params) {
  ...
  if (!membership) redirect('/manager/reps');
  ...
  const roleLabel = membership.role === 'manager_owner' ? 'Owner' : membership.role === 'manager_admin' ? 'Admin' : 'Rep';
      ...
      {!space ? (
        <div className="rounded-xl border border-dashed border-border/70 bg-muted/20 px-5 py-10 text-center">
          ...
          <p className="text-xs text-muted-foreground mt-1">
            This rep hasn&apos;t set up their workspace.
          </p>
        </div>
      ) : (
