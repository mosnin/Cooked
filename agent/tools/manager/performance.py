...
Two tools live here because they both answer "how is the team performing?":
  ...
  audit_response_times()      — per-rep median first-touch response time
                                over 30d, banded vs team median (fast /
                                on_pace / slow / no_data).
  find_at_risk_agents()       — flight-risk signals per rep: won-deal
...
async def audit_response_times(
    ...
    """Per-rep median first-touch response time over 30d, banded vs team median."""
    # Bands relative to team_median: fast (<=0.5x), on_pace (mid), slow (>=2x).
    # Reps with no outbound in window surface as no_data, not slow.
    ...
    # Per-rep audit as a coroutine — the roster runs in PARALLEL via
    # asyncio.gather instead of 2 sequential DB round-trips per rep.
    async def _audit(space: dict[str, Any]) -> tuple[dict[str, Any], list[float]] | None:
        ...
        # a shared module would be nice, but the rep tools follow the
    ...
    per_rep: list[dict[str, Any]] = [r[0] for r in audited if r is not None]
    ...
    if team_median is None:
        # No outbound across the team — return everyone in no_data.
        for r in per_rep:
...
# Silence threshold: if a rep has had no activity (Contact or Deal
...
async def find_at_risk_agents(
    ...
    """Flight-risk signals per rep: won-deal trend, silence days, un-worked leads, SLA hits."""
    # Signals per rep:
    ...
    # Reps with ≥1 signal surface in the at_risk list, sorted by signal severity.
    require_manager_role(ctx)
    team_id = ctx.context.team_id
    ...
    # Per-rep assessment as a coroutine so the whole roster runs in
    # PARALLEL via asyncio.gather. The sequential version paid ~6 DB
    # round-trips per rep in series — 120 sequential queries for a
    # 20-agent team, the dominant term in "manager chat is slow".
    async def _assess(space: dict[str, Any]) -> dict[str, Any] | None:
        ...
        if not reasons:
            return None  # no signals — rep is fine
