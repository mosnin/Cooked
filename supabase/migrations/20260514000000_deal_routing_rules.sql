-- ============================================================================
-- Team Deal Routing Rules: Schema (Phase BP7d — rules layer)
-- ============================================================================
...
--   1. Load DealRoutingRule rows WHERE teamId=? AND enabled=true
...
--      the agent is still eligible (active member of this team with
...
--      Team.assignmentMethod behaviour (round_robin / score_based /
...
CREATE TABLE IF NOT EXISTS "DealRoutingRule" (
  ...
  -- that agent is no longer eligible (offboarded, left the team, no
  ...
  -- the tag is NULL the pool is the full rep_member set.
  --
  -- NOTE: as of this migration, neither User nor TeamMembership has
  ...
  -- rep pool"). A follow-up migration can add
  -- TeamMembership.tags text[] and the engine will start honouring
