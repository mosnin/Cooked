...
export default async function TeamInvitesPage({
  ...
  try {
    ...
    console.error('[settings/team] Failed to fetch user', err);
    return (
      <div className="flex min-h-[40vh] items-center justify-center">
        <div className="text-center space-y-3 p-8">
          ...
          <a href={`/s/${slug}/settings/team`} className={PRIMARY_PILL}>
            Try again
          </a>
        </div>
      </div>
  ...
  try {
    const { data, error } = await supabase
      .from('Invitation')
      .select('id, email, roleToAssign, token, status, expiresAt, createdAt, Team(id, name)')
    ...
    console.error('[settings/team] Failed to fetch invitations', err);
  }
  ...
  const narration =
    invitations.length === 0
      ...
      : invitations.length === 1
        ...
        : `${invitations.length} team invites waiting.`;
    ...
    <div className={`${SECTION_RHYTHM} ${READING_MAX} pb-12`}>
      ...
      {invitations.length === 0 ? (
        <div className="rounded-md border border-border/70 bg-background px-5 py-12 text-center space-y-1">
          ...
          <p className={CAPTION}>
            When a team invites you, it shows up here.
          </p>
        </div>
      ) : (
        <div>
          {invitations.map((inv) => {
            const teamName = Array.isArray(inv.Team)
              ? (inv.Team as Array<{ name?: string }>)[0]?.name
              : inv.Team?.name;
              ...
              >
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    <p className={`${BODY} font-medium truncate`}>{teamName ?? 'Unknown team'}</p>
