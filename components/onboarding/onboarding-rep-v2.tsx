...
/**
 * Rep onboarding - V2, the conversational flow.
 *
 * The rep doesn't fill out a form; they have a conversation with Koala,
 ...
 *   Act II (chat)   - Koala types each question; the rep answers through
 ...
 * call (check_slug, save_profile, create_space, save_rep_profile,
 ...
 * (`onboarding-rep.tsx`) stays the rollback path - don't refactor it.
 */
...
const ROLE_LABEL: Record<Role, string> = {
  ...
  team_owner: 'I own the team',
};

export function OnboardingRepV2({ defaultName }: Props) {
  ...
  // Auto-derive slug from business name until the rep edits it.
  useEffect(() => {
  ...
  const handleCreateWorkspace = useCallback(async () => {
    ...
    try {
      ...
      await fetch('/api/onboarding', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action: 'save_rep_profile', role, zipCode, yearsExperience: TENURE_TO_YEARS[tenure] }),
      });
  ...
  const handleFinish = useCallback(async () => {
    ...
    try {
      ...
      if (role === 'team_owner') {
        ...
        const managerData = (await managerRes.json().catch(() => ({}))) as { team?: { id?: string } };
        const newTeamId = managerData.team?.id;
        if (newTeamId) {
          await fetch('/api/spaces', {
            method: 'PATCH', headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ slug, teamId: newTeamId }),
          }).catch(() => undefined);
        }
      ...
      const completeRes = await fetch('/api/onboarding', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action: 'complete', accountType: 'rep' }),
      });
      if (!completeRes.ok) throw new Error('complete');
  ...
  // The rep's answer bubble for a completed step. null = no bubble.
