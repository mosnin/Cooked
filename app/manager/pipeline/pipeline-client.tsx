...
export function PipelineClient({ deals, stages, reps, summary }: Props) {
  ...
  const filteredDeals = useMemo(() => {
    return deals.filter((d) => {
      ...
      if (repFilter !== 'all' && d.agentUserId !== repFilter) return false;
      return true;
    });
  }, [deals, statusFilter, repFilter]);
    ...
    <div className="space-y-10">
      ...
      <section className="space-y-4">
        ...
        {agentGroups.length === 0 ? (
          <DashedEmpty>No reps yet.</DashedEmpty>
        ) : (
          <ul className="divide-y divide-border/60">
            {agentGroups.map((agent) => {
              const isExpanded = expandedReps.has(agent.userId);
              ...
              return (
                <li key={agent.userId}>
                  <button
                    onClick={() => toggleRep(agent.userId)}
