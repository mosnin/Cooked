 ...
 *
 * Replaces the old snapshot-counts pattern ("5 reps, 2 drafts, 3 hot
 ...
 * deep-links into /manager/koala with the prompt prefilled — making the
...
export function ManagerMorningStory() {
  ...
  function handleTap() {
    ...
    router.push(
      `/manager/koala?prefill=${encodeURIComponent(suggestedPrompt)}`,
    );
  }
...
/**
 * Hairline-divider context row. Three numbers — reps, active deals,
...
function StatsRow({
  ...
  return (
    <div className="flex items-center gap-4 text-[11px] tabular-nums text-muted-foreground">
      <Stat label="reps" value={loading ? '—' : `${stats.reps}`} />
      <Divider />
