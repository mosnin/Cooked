...
import { sweepAllTeams } from '@/lib/manager-sla';
 ...
 *
 * Runs every 15 minutes (see vercel.json). For each team with SLA
 * enforcement on, it finds routed leads sitting un-worked past the first-
 * response window and acts: nudges the assigned rep, then escalates to the
...
async function handler(req: NextRequest) {
  ...
  try {
    const results = await sweepAllTeams();
    ...
    logger.info('[cron/lead-sla] sweep complete', { teams: results.length, ...totals });
    return NextResponse.json({ ok: true, teams: results.length, ...totals });
  } catch (err) {
