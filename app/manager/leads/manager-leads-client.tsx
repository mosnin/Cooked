...
function UnassignedRow({
  ...
  lead: LeadRow;
  reps: RepOption[];
  onAssigned: (leadId: string, rep: RepOption) => void;
  ...
  async function handleAssign(rep: RepOption) {
    setAssigning(true);
    try {
      const res = await fetch('/api/manager/assign-lead', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ contactId: lead.id, repUserId: rep.userId }),
      });
      if (!res.ok) {
        const data = await res.json().catch(() => ({}));
        throw new Error(data.error || "I couldn't route that lead.");
      }
      toast.success(`Routed to ${rep.name ?? rep.email}.`);
      onAssigned(lead.id, rep);
    } catch (err: unknown) {
    ...
    <li>
      <div className="group/row flex items-center gap-3 py-3 px-2 -mx-2 rounded-md hover:bg-muted/30 transition-colors">
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          ...
          <RepPicker reps={reps} onSelect={handleAssign} disabled={assigning} />
          {confirmDelete ? (
...
function AssignedRow({
  ...
  onUnassigned?: (leadId: string, repName: string) => void;
}) {
  ...
  const repName = progress?.repName ?? lead.assignedTo ?? 'this rep';
  ...
  async function handleUnassign() {
    ...
    try {
      ...
      toast.success(`Pulled back from ${repName}.`);
      setConfirmOpen(false);
      onUnassigned?.(lead.id, repName);
    } catch (err: unknown) {
      ...
      <Dialog open={confirmOpen} onOpenChange={setConfirmOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              {lead.name || 'This lead'} comes off {repName}&rsquo;s plate and lands back in the unassigned queue.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <button
        ...
        <div className="pb-3 pl-[calc(32px+0.75rem)]">
          <div className="rounded-lg bg-muted/30 border border-border/60 p-3 space-y-3">
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs">
              <div>
                <p className={SECTION_LABEL}>Rep</p>
                <p className="mt-0.5 font-medium text-foreground">{progress.repName}</p>
              </div>
              <div>
...
export function ManagerLeadsClient({
  ...
  // Resolve the rep a routed lead sits with — same rule AssignedRow uses
  // (live progress wins, falls back to the assignment label).
  const repOf = useCallback(
    (lead: LeadRow) => assignedLeadProgress[lead.id]?.repName ?? lead.assignedTo ?? null,
    [assignedLeadProgress],
  );

  // Reps who actually have routed leads, plus any team member — so the
  // menu mirrors the team even before everyone has a lead. Sorted, deduped.
  const repNames = useMemo(() => {
    const set = new Set<string>();
    for (const lead of assigned) {
      const name = repOf(lead);
  ...
  const filteredUnassigned = useMemo(() => {
    // A specific rep has no claim on unassigned leads — hide them.
    if (repFilter !== 'all') return [];
    return applySearch(unassigned);
  }, [unassigned, applySearch, repFilter]);

  const filteredAssigned = useMemo(() => {
    const byRep =
      repFilter === 'all'
        ? assigned
        : assigned.filter((l) => repOf(l) === repFilter);
    return applySearch(byRep);
  }, [assigned, applySearch, repFilter, repOf]);

  function handleAssigned(leadId: string, rep: RepOption) {
  ...
  return (
    <div className="space-y-8 pb-56 md:pb-24">
      {/* Search + by-rep filter — the only chrome. View toggles, sort
      ...
      {/* Fresh-team empty state — no leads at all. Canonical dashed card,
      ...
      {/* Unassigned — the one demanding routing. Shown first so it's the
          ...
          single rep: unassigned leads belong to no one, so an empty
          queue under a rep filter is noise. */}
      {hasAnyLeads && repFilter === 'all' && (
        <section className="space-y-2">
          <SectionHeader
            label="Unassigned"
            count={filteredUnassigned.length}
            hint={filteredUnassigned.length > 0 ? 'Route to a rep' : undefined}
          />
            ...
            <div className="rounded-xl border border-dashed border-border/70 bg-muted/20 px-5 py-10 text-center">
              <p className="text-sm text-foreground">
                {isSearching
                  ...
                  : repFilter !== 'all'
                    ? `Nothing on ${repFilter}’s plate yet.`
                    : "Nothing on a rep's plate yet."}
              </p>
            </div>
          ) : (
            <ul className="divide-y divide-border/60">
