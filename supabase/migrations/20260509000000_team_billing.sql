-- ============================================================================
-- Team Billing: Per-Team Seat-Based Subscriptions
-- ============================================================================
...
--
-- Moving the subscription, plan, and seatLimit onto Team itself closes
-- all three gaps: seats are counted against the team row, usage is
...
-- solo reps (non-manager accounts with no Team) still bill through
-- their Space, and the Stripe webhook will route incoming events by metadata
-- to either a Team or a Space depending on which flow created the sub.
-- ============================================================================

-- 1. Extend Team with billing + plan columns
ALTER TABLE "Team"
...
-- 3. Bootstrap existing teams' billing from their manager_owner's personal
--    Space subscription — but ONLY for rows that don't already have a
--    subscription on the Team side. One-shot, idempotent by the guard.
--    Space.stripe* columns are intentionally left intact; solo reps (users
--    with no Team) still bill through their Space.
UPDATE "Team" b
...
--    no Stripe ids). Stripe-webhook agent: use these to resolve a Team
