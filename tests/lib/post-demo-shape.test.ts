...
describe('shapeProposals — integration tools', () => {
  ...
  it('keeps integration slugs that match the rep\'s connected apps', () => {
    const out = shapeProposals(
      {
        proposals: [
  ...
  it('drops integration slugs the rep has NOT connected', () => {
  ...
  it('produces a clean rep-voice summary without "via Gmail"', () => {
    const out = shapeProposals(
      { proposals: [{ tool: 'GMAIL_SEND_EMAIL', args: { to: 'sam@chen.com' } }] },
      { integrationTools: [GMAIL] },
    );
    expect(out[0].summary).not.toMatch(/via gmail/i);
    // Verb is the truth: "Email" tells the rep what's happening.
...
describe('repVerbForToolkit / doneVerbForToolkit', () => {
  it('returns concise verbs for known toolkits', () => {
    expect(repVerbForToolkit('gmail')).toBe('Email');
    expect(repVerbForToolkit('googlecalendar')).toBe('Add to calendar');
  ...
  it('falls back gracefully for unknown toolkits', () => {
    expect(repVerbForToolkit('weird-app')).toBe('Run connected action');
