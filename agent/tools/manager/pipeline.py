...
momentum?":
  ...
  find_stuck_deals(min_days)         — active deals across the team that
                                       haven't been touched in `min_days`+
                                       days, ranked by age × value so the
                                       biggest stuck deals surface first.
  find_unassigned_leads(min_hours)   — team-intake leads sitting in the
                                       queue without a rep on them, with
  ...
  find_breached_leads()              — routed leads (tag: assigned-by-manager)
                                       ...
                                       the team's slaFirstResponseMinutes
                                       clock; flags any already past
                                       slaEscalateMinutes too.

Both are team-scoped through `Space WHERE teamId = ctx.team_id`
and gated by `require_manager_role(ctx)`. Phase 3 will add the matching write
tools (assign_lead, ping_rep_on_stuck_deal); for now these are read-only
...
# Default SLA window when the team row has no value set.
...
async def find_stuck_deals(
    ...
    """Find active deals across the team stalled min_days+, ranked by days*value."""
    ...
    # Resolve the team's spaces in one query — we filter Deal on
    ...
    # Resolve rep names for the assigned_to field.
    ...
    for d in deals:
        ...
        scored.append({
            ...
            "assigned_to": {
                "id": owner_id or "",
                "name": (owner or {}).get("name") or (owner or {}).get("email") or "Rep",
...
async def find_unassigned_leads(
    ...
    """Find unassigned team-intake leads, with the lightest-load rep proposed."""
    # Unassigned = Contact in this team tagged 'team-lead', not 'assigned', older than min_hours.
    ...
    # Compute open_load per rep — duplicate of team_health's logic so
    ...
    # Sort ascending by open_load; the lightest rep is at the top.
    rep_loads.sort(key=lambda r: r["open_load"])
    lightest = rep_loads[0]

    # Team median for the "vs benchmark" framing in the reason string.
    loads = [r["open_load"] for r in rep_loads]
...
async def find_breached_leads(
    ...
    """Find routed leads past the team's first-response SLA, oldest first."""
    ...
    sla_first_minutes: int = int(
        team.get("slaFirstResponseMinutes") or _DEFAULT_SLA_FIRST_RESPONSE_MINUTES
    )
    sla_escalate_minutes: int | None = (
        int(team["slaEscalateMinutes"])
        if team.get("slaEscalateMinutes") is not None
        else None
    )
    ...
    if not spaces:
        return {
            "ok": True,
            "breached_leads": [],
            "summary": "No member spaces found for this team.",
        }
    ...
    for c in breached:
        ...
        owner = users_by_id.get(owner_id) if owner_id else None
        rep_name = (
            (owner or {}).get("name") or (owner or {}).get("email") or "Unknown Rep"
        )
