...
/**
 * ManagerPropertiesClient — the interactive shell for /manager/properties.
 *
 ...
 *   1. Ownership badge — each row shows the assigned rep name (or
 *      "Available" when unassigned) so the manager can scan at a glance.
 *   2. Assign control — a compact native <select> that PATCHes
 *      /api/manager/properties/[id]/assign on change. Stops row click
 *      propagation so hover state doesn't confuse intent.
 *   3. Add property panel — inline form that POSTs to /api/manager/properties
 *      with an assign-on-create field. Toggled from the header CTA.
 *
 * API contract (unchanged):
 *   GET  /api/manager/properties         -> { properties, members }
 *   POST /api/manager/properties         -> Property
 *   PATCH /api/manager/properties/[id]/assign -> Property
 */
...
function AssignControl({ property, members, onAssigned }: AssignControlProps) {
  ...
  async function handleChange(e: React.ChangeEvent<HTMLSelectElement>) {
    ...
    setSaving(true);
    try {
      const res = await fetch(`/api/manager/properties/${property.id}/assign`, {
        method: 'PATCH',
...
// Cannot reuse PropertyForm directly because the manager form adds an
  ...
  return (
    <div className="rounded-xl border border-border/70 bg-card px-5 py-5">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        {/* Assign on create — manager-only field not in PropertyForm */}
        {members.length > 0 && (
          <FormField label="Assign to rep">
            <select
...
export function ManagerPropertiesClient() {
  ...
  return (
    <div className="space-y-6">
      {/* Page header — status-sentence pattern matching the rep properties
      ...
      {properties.length === 0 ? (
        <div className="rounded-xl border border-dashed border-border/70 bg-muted/20 px-5 py-12 text-center">
          ...
          <p className={cn('text-xs mt-1', BODY_MUTED)}>
            Add the first listing to start assigning to your reps.
          </p>
          <button
            type="button"
        ...
        /* divide-y row list — pixel-matches the rep properties page.
            ...
            return (
              <StaggerItem key={property.id}>
                <div className="flex items-center gap-4 py-4 -mx-2 px-2 rounded-md hover:bg-foreground/[0.04] transition-colors">
                  {/* Thumbnail — 128px wide, 4:3 aspect, matches rep page */}
                  ...
                  <div className="flex-1 min-w-0 space-y-1">
                    ...
                    <div className="flex items-center gap-2 pt-0.5 flex-wrap">
                      ...
                      {/* Ownership — always present. "Available" signals an
                          unassigned pool slot; a rep name signals it is
