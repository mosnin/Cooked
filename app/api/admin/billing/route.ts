 ...
 * requirePlatformAdmin(). The account is { space | team, id }; credit ops
...
function parseAccount(type: unknown, id: unknown): BillingAccount | null {
  if ((type === 'space' || type === 'team') && typeof id === 'string' && id) {
    return { type, id };
  }
  return null;
}
...
async function lookupPlan(acc: BillingAccount): Promise<string | null> {
  const table = acc.type === 'space' ? 'Space' : 'Team';
...
export async function GET(req: Request) {
  ...
  if (!account) {
    return NextResponse.json({ error: 'accountType (space|team) and accountId are required' }, { status: 400 });
  }
  try {
