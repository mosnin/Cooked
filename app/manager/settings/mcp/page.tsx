...
import { TeamMcpSection } from '../mcp-section';
import {
  H1,
  TITLE_FONT,
  BODY_MUTED,
    ...
    return (
      <div className={`${SECTION_RHYTHM} ${READING_MAX} pb-56 md:pb-24`}>
        ...
        <p className={BODY_MUTED}>
          Only the team owner or admins can manage MCP keys.
        </p>
      </div>
    );
  }
    ...
    return (
      <div className={`${SECTION_RHYTHM} ${READING_MAX} pb-56 md:pb-24`}>
        <header className="space-y-1.5">
          ...
          <p className={BODY_MUTED}>
            Connect external AI tools to your team data via MCP.
          </p>
        </header>
        <div className="rounded-xl border border-dashed border-border/70 bg-muted/20 px-5 py-10 text-center">
          <p className="text-sm text-foreground">MCP isn&apos;t available yet.</p>
          <p className="text-xs text-muted-foreground mt-1">
            The team owner needs a workspace before keys can be generated.
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className={`${SECTION_RHYTHM} ${READING_MAX} pb-56 md:pb-24`}>
      <header className="space-y-1.5">
        ...
        <p className={BODY_MUTED}>
          Connect external AI tools (Claude, Cursor, Windsurf) to {team.name}.
        </p>
      </header>

      <TeamMcpSection slug={managerSpaceSlug} />
    </div>
  );
}
