...
| Manager dashboard, team roster, lead routing, commissions, reviews, invites, seats | **7. Team Tier** |
| Applicant/client portal login, messaging a rep, client documents | **8. Client Portal** |
...
- **Owns:** `User` (`clerkId`, `status`, `platformRole`), `TeamMembership`, `Space.ownerId`.
...
- **Entry:** `app/api/public/apply/route.ts`, `app/api/public/apply/team/route.ts`, `app/api/public/intake-chat/route.ts`, `app/apply/[slug]/**`; scoring: `lib/lead-scoring.ts` → `lib/scoring/engine.ts` (deterministic) + `lib/scoring/enhance.ts` (optional LLM summary, `gpt-4.1-mini`).
- **Owns:** `Contact`, `ApplicationStatusUpdate`, `FormDraft`; reads `Space`, `SpaceSetting`, `Team`.
...
Multi-agent org oversight: membership, lead routing, commissions, deal reviews, templates, invitations, seats. Manager↔rep isolation is the recurring bug class (PRs #294–#296).

- **Entry:** `lib/permissions.ts` (`requireManager`/`getManagerContext`/`getManagerMemberContext`), `lib/team-routing.ts`, `lib/team-seats.ts`, `app/api/manager/**`, `app/api/invitations/[token]/route.ts`.
- **Owns:** `Team`, `TeamMembership`, `Invitation`, `TeamRemoval`, `CommissionLedger`, `DealRoutingRule`, `TeamTemplate`, `DealReviewRequest/Comment`, `ManagerNotification`.
- **Fragile seams:** (1) A manager route must verify the caller belongs to the **specific** team that owns the resource, not just "is a manager." (2) `getManagerContext` multi-membership tie-break is order-dependent. (3) Seat-cap + pending-invite count has no DB-level serialization (double-spend race); invitation accept re-activates offboarded users by design.
...
- **Owns:** `Space.stripe*`/`trialUsedAt`, `Team.stripe*`/`plan`/`seatLimit`, `AuditLog`, `DeadLetterEvent`, `PushSubscription`, `CallLog`.
...
1. **Isolation-by-title-prefix is not isolation.** Manager-Koala / team-chat conversations once lived in the rep `Conversation`/`Message` tables distinguished only by a `[MANAGER_KOALA]`/`[TEAM_CHAT]` title prefix. Because a manager_owner also owns their personal rep space, *space ownership alone is not a boundary*. Any rep read of `Conversation`/`Message` MUST route through `lib/chat/conversation-access` (`isRepConversation` / reserved-prefix exclusion) — list, single-conversation, messages, **and the write path** (`resolveConversation`). Manager chat now has its own `ManagerConversation`/`ManagerMessage` tables (keyed by `teamId`); prefer structural separation over string matches.
2. **`Space.teamId` is NOT "which team the user belongs to."** Membership lives in `TeamMembership` (a user can be in several). `Space.teamId` is only the intake-config owner and must never be overwritten on a second join. Don't read it as a membership signal.
3. **Parallel rep↔manager routes drift.** `task`/`manager-task`, `conversations`/`manager-conversations`, `messages`/`manager-messages` are near-duplicates; a fix applied to one is easily missed on the other (the isolation guard gap lived in exactly this seam). When you touch one, check its twin.
...
5. **Service-role bypasses RLS.** All server queries use the service-role key, so RLS is *not* the tenant boundary — app-layer `.eq('spaceId'/'teamId')` scoping is. Add the scope filter; don't rely on a policy.
