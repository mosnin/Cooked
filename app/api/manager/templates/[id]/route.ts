...
export async function PATCH(req: NextRequest, { params }: Params): Promise<NextResponse> {
  ...
  // Confirm the template is in the caller's team before touching it.
  // A missing row is a 404 regardless of whether it exists for another
  // team — we don't want to leak cross-team existence.
  ...
  const patch: Partial<TeamTemplateRow> & { updatedAt: string } = {
 ...
 *
 * Removes the TeamTemplate row. Agent-local copies (MessageTemplate rows
