 ...
 * Default status = 'open'. Returns up to 200 rows belonging to this team.
...
export async function GET(req: NextRequest) {
  ...
  let query = supabase
    ...
    .select(
      'id, dealId, status, reason, createdAt, resolvedAt, resolvedNote, requestingUserId, teamId',
    )
  ...
  if (rowsErr) {
    logger.error('[manager/reviews/GET] list failed', { teamId: ctx.team.id }, rowsErr);
  ...
  if (usersRes.error) {
    logger.error('[manager/reviews/GET] user fetch failed', { teamId: ctx.team.id }, usersRes.error);
    return NextResponse.json({ error: 'Failed to load reviews' }, { status: 500 });
  }
  if (dealsRes.error) {
    logger.error('[manager/reviews/GET] deal fetch failed', { teamId: ctx.team.id }, dealsRes.error);
    return NextResponse.json({ error: 'Failed to load reviews' }, { status: 500 });
  }
  if (commentsRes.error) {
    logger.error('[manager/reviews/GET] comment count failed', { teamId: ctx.team.id }, commentsRes.error);
    return NextResponse.json({ error: 'Failed to load reviews' }, { status: 500 });
  }
  ...
  if (spacesRes.error) {
    logger.error('[manager/reviews/GET] space fetch failed', { teamId: ctx.team.id }, spacesRes.error);
    return NextResponse.json({ error: 'Failed to load reviews' }, { status: 500 });
  }
