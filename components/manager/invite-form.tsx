...
interface InviteFormProps {
  isOwner?: boolean;
  /**
   * Current seat usage for the team. When present and well-formed, an
...
export function InviteForm({ isOwner = true, seatUsage }: InviteFormProps) {
  ...
  const [role, setRole] = useState<'rep_member' | 'manager_admin'>('rep_member');
    ...
    <form onSubmit={handleSubmit} className="space-y-3">
      <div className="flex flex-col sm:flex-row gap-2">
        ...
        {isOwner ? (
          ...
          <input type="hidden" name="role" value="rep_member" />
        )}
        <Button
          type="submit"
          size="sm"
      ...
      {role === 'manager_admin' && isOwner && (
        <p className="text-xs text-muted-foreground flex items-center gap-1.5">
          <ShieldCheck size={12} />
          Admins can manage members, leads, and team settings.
        </p>
      )}
      {message && (
        <div
