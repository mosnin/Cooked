...
interface Props {
  ...
  onShowFullDay: () => void;
  /**
   * True only when the rep has done absolutely nothing yet — no contacts,
 ...
 *
 * Cascade: drafts (most urgent) → questions (next-most). Rep sees ONE
 ...
 * rotates out of view now. Reps should never feel forced to act on
...
export function FocusCard({
  ...
  // rep rotates to a new card. Used to compute time-to-decision (an
  // important feedback signal — fast send = trust the model, slow send = the
  // model is making the rep think too hard). Per-card so a held card
  ...
  // Also reset the view timestamp — the rep is "first seeing" this item
  // now, even if it was queued an hour ago server-side. Decision latency is
  // the time the *rep* spent looking at the card, not the time the draft
  // sat in the queue.
  useEffect(() => {
  ...
  function hold() {
    ...
    // the rep from rotating. Questions don't get this ping (different
  ...
  // Items exist but rep has held them all in this session
  if (!current) {
    return (
      <FocusEmpty
