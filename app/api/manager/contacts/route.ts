 ...
 *
 * Returns contacts scoped to ALL member spaces in the manager's team,
 * each annotated with `repName` (the owning rep's display name).
 *
 * Query params:
...
export async function GET(req: NextRequest) {
  ...
  // ── Build spaceId → rep display-name map ──────────────────────────────
  const spaceToRep = new Map<string, string>();
  for (const m of allMembers) {
    const sid = m.Space?.id;
    if (!sid) continue;
    spaceToRep.set(sid, m.User?.name ?? m.User?.email ?? 'Unknown rep');
  }
  ...
  // Annotate each row with repName
  const annotated = (data ?? []).map((c: any) => ({
    ...c,
    repName: spaceToRep.get(c.spaceId) ?? 'Unknown rep',
  }));
