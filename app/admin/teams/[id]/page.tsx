...
export async function generateMetadata({ params }: Params) {
  ...
  const { data } = await supabase.from('Team').select('name').eq('id', id).maybeSingle();
  return { title: `${data?.name ?? 'Team'} — Admin` };
}

export default async function AdminTeamDetailPage({ params }: Params) {
  const { id } = await params;
  ...
  const [dealRows, pendingInviteCount] = await Promise.all([
    spaceIds.length > 0
      ...
      : Promise.resolve([]),
    supabase.from('Invitation').select('*', { count: 'exact', head: true }).eq('teamId', id).eq('status', 'pending'),
  ]);
  ...
  const roleLabel = (role: string) =>
    role === 'manager_owner' ? 'Owner' : role === 'manager_admin' ? 'Admin' : 'Rep';
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className={H1} style={TITLE_FONT}>{team.name}</h1>
              ...
              <CardContent className="px-4 py-3 space-y-3">
                <div className="flex items-start gap-2.5">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-xs font-mono break-all">{team.id}</p>
                  </div>
                </div>
                <div className="flex items-start gap-2.5">
                ...
                {team.joinCode && (
                  <div className="flex items-start gap-2.5">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-xs font-mono">{team.joinCode}</p>
                    </div>
                  </div>
                )}
                {team.websiteUrl && (
                  <div className="flex items-start gap-2.5">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-xs truncate">{team.websiteUrl}</p>
                    </div>
                  </div>
                )}
                <div className="flex items-start gap-2.5">
      ...
      <div className="space-y-2">
        ...
        {invitations.length === 0 ? (
          <div className="rounded-xl border border-dashed border-border/70 bg-muted/20 px-5 py-10 text-center">
            ...
            <p className="text-xs text-muted-foreground mt-1">
              Invitations this team sends will show up here.
            </p>
          </div>
        ) : (
          <ul className="divide-y divide-border/60 border-y border-border/60">
                ...
                <li key={inv.id} className="flex items-center gap-3 py-3">
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2 min-w-0 flex-wrap">
                      ...
                      <span className="text-[11px] font-medium text-muted-foreground bg-muted rounded-full px-2 py-0.5 flex-shrink-0">
                        {inv.roleToAssign === 'manager_admin' ? 'Admin' : 'Rep'}
                      </span>
