 ...
 * the team pool for Team/Team Plus (resolveBillingAccount handles that).
 */
export async function POST(req: NextRequest) {
  try {
    ...
    // Which balance does this space fund? Team/Team Plus pool at the team.
    const { account } = await resolveBillingAccount(space.id);

    // Use the FUNDING account's Stripe customer, not the space's. For a
    // Team/Team Plus space the balance pools at the team, whose customer
    // lives on Team.stripeCustomerId — the space's own customer is usually
    ...
    const customerTable = account.type === 'team' ? 'Team' : 'Space';
