...
export function ManagerPeopleTable() {
  ...
  const subtitle = (() => {
    ...
    if (newThisWeekCount > 0) {
      return `${contacts.length} ${noun} across the team. ${newThisWeekCount} new this week.`;
    }
    return `${contacts.length} ${noun} across the team.`;
  })();
    ...
    <div className="space-y-6">
      ...
      <header className="space-y-1.5">
        ...
        <h1 className={H1} style={TITLE_FONT}>
          Everyone your team is working with
        </h1>
        ...
        if (!search && !hasAnyFilter && contacts.length === 0) {
          return (
            <div className="rounded-xl border border-dashed border-border/70 bg-muted/20 px-5 py-12 text-center">
              <p className="text-base text-foreground">No contacts across the team yet.</p>
              <p className={cn(BODY_MUTED, 'mt-1.5')}>
                Contacts will appear here as your reps add them.
              </p>
            </div>
          );
        }
...
// manager-only addition: a Rep byline under the contact's name/email line.
// Row click drills into /manager?prompt=... (Koala prefilled about this lead).
...
function ManagerContactRow({
  ...
  const drillPrompt = encodeURIComponent(
    `Tell me about ${contact.name} (${contact.repName}'s lead). Where are they in the funnel and what's the next move?`
  );
  ...
  return (
    ...
    >
      <Link href={`/manager?prompt=${drillPrompt}`} className={rowClassName}>
        ...
        {/* Name + stage pill + email/phone + rep byline */}
        <div className="min-w-0 flex-1">
          ...
          {/* Rep byline — the manager-only addition */}
