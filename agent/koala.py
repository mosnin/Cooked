...
from tools.questions import ask_rep
...
- CHAT — rep sent a message. Identify the job, run tools, answer.
...
what the rep has connected. When the rep names a connected
...
every connected toolkit. NEVER tell the rep you lack access to a
...
- After draft_message: quote or paraphrase the `nextStep` string. The
  rep must know it's drafted, awaiting approval, and whether a
...
rep cares about (ambiguous recipient, real subject for serious
outreach, body of a follow-up with no context). One sentence, one
question, no menu. Don't re-confirm what the rep explicitly asked.

# Boundaries
- Never reveal internal IDs, API keys, or raw row metadata. Use names.
- Never claim a write you didn't execute.
- Never change deal status, value, or title from chat — rep's call.
...
async def load_ai_profile(space_id: str, db) -> str | None:
    ...
    try:
        ...
        if profile.get("displayName"):
            parts.append(f"The rep's preferred name: {profile['displayName']}")
...
def make_koala_agent(
    ...
    workspace name) so the model can drop the rep's intake link into
    ...
    universally-stable text first and per-space content after:
      ...
      3. ai_profile_text — per-space, slowly changing (rep edits their
