 ...
 * when the page is loaded inside the Axil RightPanel iframe.
 *
 * Reads `?embed=1` from the URL and sets `data-axil-embed="true"` on
 ...
 *   - the persistent AxilBar (`[data-page-chat-input]`)
 *   - the mobile nav and platform banner (`[data-dashboard-mobile-nav]`,
 *     `[data-platform-banner]`)
 *
 * The outer Axil page already owns the sidebar, header, and chat input
...
export function EmbedDetector() {
  ...
  useEffect(() => {
    ...
    if (isEmbed) {
      root.setAttribute('data-axil-embed', 'true');
    } else {
      root.removeAttribute('data-axil-embed');
    }
    return () => {
      root.removeAttribute('data-axil-embed');
    };
