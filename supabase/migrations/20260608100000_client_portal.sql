...
-- from the rep Clerk auth. A client signs up with email + password; once
...
--
-- These tables never touch the rep auth path. Messaging, info-requests and
-- documents hang off the existing Contact so the rep sees them in their
-- normal contact view.
...
-- ── Rep → client "please send X" requests ────────────────────────────────
