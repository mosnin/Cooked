...
export async function POST(req: NextRequest) {
  ...
  try {
    // ── Find the team owner's space ─────────────────────────────────
    const ownerSpace = await getSpaceByOwnerId(team.ownerId);
    if (!ownerSpace) {
      return NextResponse.json(
        { error: 'Team owner does not have a workspace configured.' },
        { status: 500 },
      );
    }
    ...
    // space. `routeTeamLead` never throws — null = manager fallback.
    ...
    if (routing) {
      try {
        ...
        logger.warn('[teams/leads] failed to resolve assigned agent name', {
          teamId: team.id,
    ...
    logger.info('[teams/leads] manual lead created', {
    ...
    logger.error('[teams/leads] unhandled error', {
      teamId: team.id,
    }, error);
