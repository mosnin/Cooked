...
/** A skill the rep can pick from the `/` menu. */
export interface SkillItem {
  slug: string;
  title: string;
  description: string;
  /** Composer text; may carry one {placeholder} for the rep to fill. */
  prompt: string;
}
 ...
 * the rep types straight over it; with no token the cursor lands at the
...
interface KoalaPromptBoxProps {
  ...
  skills?: SkillItem[];
  /**
   * Show the Chat ↔ Agent runtime switch. Default true (rep surface).
...
export const KoalaPromptBox = React.forwardRef<HTMLTextAreaElement, KoalaPromptBoxProps>(
  function KoalaPromptBox(
    ...
    // text-keyed so the rep can edit/clear without us re-stomping it.
    ...
    // textarea keep it open — the rep is still typing the query.
    useEffect(() => {
      if (!slashOpen) return;
      const onDown = (e: MouseEvent) => {
    ...
    function handleSubmit() {
      ...
      // next message doesn't silently run the tool loop the rep forgot
      // they'd left on.
      setChatMode('chat');
