...
interface Props {
   ...
   *  request body. The recorder UI is unchanged; the rep never sees
   *  the id. */
  personId?: string;
...
export function PostDemoRecorder({ slug, personId, dealId }: Props) {
  ...
  // The rep never sees these — they're a hint to the proposal model so
...
/** Keys we never expose to the rep in the args editor — they're
...
const HIDDEN_ARG_KEYS = new Set(['personId', 'dealId', 'contactId', 'propertyId', 'demoId']);
...
function ArgsEditor({
  ...
  // synthesize new keys — the model already chose them; the rep edits
