 ...
 * because Koala creates demo events autonomously; here the rep is
...
export interface MailConnection {
  ...
  id: string;
  /** Composio entityId — the rep's Clerk userId. */
  userId: string;
  /** Provider slug. */
...
/**
 * Find the rep's active email connection for this space. Returns the
 * first match across the mail toolkits — Gmail wins over Outlook if both
 * are connected (Gmail is the dominant rep inbox; same precedence as
 * `lib/delivery.ts`).
 *
...
/**
 * Find the rep's active WhatsApp Business connection for this space.
 *
...
/**
 * Send an email through the rep's provider. Mirror of
 ...
 * (see app/api/email/send/route.ts). If the rep only has Outlook
...
/**
 * Star / unstar an email in the rep's provider.
 *
...
/**
 * Send a WhatsApp message through the rep's Business account.
 *
 ...
 * surface the error verbatim — the surface tells the rep honestly
