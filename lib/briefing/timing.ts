/**
 * Per-rep brief timing — translates the cron's UTC tick into "is it
 * this rep's briefing hour right now, in their own timezone."
 *
 ...
 *
 * Why per-rep local time: a 7am brief at UTC is 2am in Pacific time.
 * The rep wakes up to a brief from yesterday. The Vitruvian principle
 * for time: the rep's morning is the unit, not the server's clock.
 */
 ...
 * Used as the `forDate` key on Brief rows so the rep's "today's brief"
