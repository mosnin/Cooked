...
describe('getSeatUsage', () => {
  it('sums members + non-expired pending invites', async () => {
    mockByTable = {
      Team: { single: { plan: 'team', seatLimit: 5 } },
      TeamMembership: { count: 7 },
      Invitation: { count: 3 },
    };
  ...
  it('derives seatLimit from the plan when the column is null (team_plus → 10)', async () => {
    mockByTable = {
      Team: { single: { plan: 'team_plus', seatLimit: null } },
      TeamMembership: { count: 42 },
      Invitation: { count: 0 },
    };
    const u = await getSeatUsage('b1');
  ...
  it('falls back to team/5 when the Team row select errors (pre-migration)', async () => {
    mockByTable = {
      Team: { error: { message: 'column "plan" does not exist' }, single: null },
      TeamMembership: { count: 2 },
      Invitation: { count: 1 },
    };
    const u = await getSeatUsage('b1');
    // Fail CLOSED on the cap — the helper should NOT unlock the team
...
describe('checkSeatCapacity', () => {
  ...
  it('team_plus honors its larger cap (used=9/10 allows +1)', async () => {
    mockByTable = {
      Team: { single: { plan: 'team_plus', seatLimit: 10 } },
      TeamMembership: { count: 9 },
      Invitation: { count: 0 },
    };
    const r = await checkSeatCapacity('b1', 1);
    expect(r.ok).toBe(true);
  ...
  it('fails CLOSED on infra count error — silent overages are worse than a transient 402', async () => {
    ...
    // team silently exceeding its billed seat count.
    mockByTable = {
      Team: { single: { plan: 'team', seatLimit: 5 } },
      TeamMembership: { error: { message: 'db flap' }, count: null },
      Invitation: { count: 0 },
    };
    const r = await checkSeatCapacity('b1', 1);
    expect(r.ok).toBe(false);
