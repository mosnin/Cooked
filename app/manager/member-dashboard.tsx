...
export async function MemberDashboard({ ctx }: MemberDashboardProps) {
  ...
  const userName = userRow?.name ?? userRow?.email ?? 'Rep';
      ...
      <div className="space-y-6 w-full">
        <header className="space-y-1.5">
          ...
          <p className={cn(BODY_MUTED)}>
            {`${team.name} · finish your workspace to start tracking leads.`}
          </p>
        </header>
  ...
  // Resolve the team's admin/owner spaces FIRST so the announcement
  ...
  // rows over the wire.
  const managerMembers = await getTeamMembers(team.id);
  ...
  const statusSentence = (() => {
    ...
    if (parts.length === 0) {
      return `${team.name} · quiet day — nothing in flight.`;
    }
