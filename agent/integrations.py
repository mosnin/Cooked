...
_NATIVE_TOOL_NAMES = frozenset({
    ...
    "request_deal_review",
    "book_demo", "route_lead", "add_property", "send_property_packet",
    ...
    "process_inbound_message", "read_attachment", "ask_rep",
...
async def active_toolkits(space_id: str, user_id: str) -> list[str]:
    """Toolkit slugs the rep has connected and our DB has marked active.
...
def _build_curated_tool(
    *,
    ...
    runtime behavior, error shapes, and the rep's `IntegrationConnection`
...
async def load_integration_tools(
    ...
    proxy not configured — tell the rep…"), which the model can relay
    as "temporarily unavailable". Returning [] here made every config
    failure indistinguishable from "the rep connected nothing", and
    the model told reps their integrations were gone.
    """
    proxy = _proxy_base()
    if proxy is None:
        logger.error(
    ...
    # dispatcher-only default meant a rep's connected Gmail surfaced as
    ...
    # Collect curated slugs across the rep's connected toolkits. A
