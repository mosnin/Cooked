 ...
 *   The rep's UI POSTs the decision to
...
function buildSseStream(input: BuildStreamInput): ReadableStream<Uint8Array> {
  return new ReadableStream<Uint8Array>({
    async start(controller) {
      ...
      // call's telemetry — the sentence the rep sees before the
      ...
      const pushEvent = (event: PushableEvent) => {
        ...
        if (event.type === 'tool_call_result') {
          ...
          // marker, strip the marker so the rep sees clean text, push a
      ...
      try {
        ...
        // to the browser, so the rep SAW the reply. If we just log and
        ...
        // failure surfaces as an error event so the rep knows the
 ...
 * the rep-facing approval prompts. Returns the new row id, or null
