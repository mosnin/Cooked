 ...
 * active team and emails it to the manager owner.
 *
...
async function handler(req: NextRequest) {
  ...
  try {
    ...
    const { data: teams, error: brokErr } = await supabase
      .from('Team')
    ...
    if (!teams?.length) {
      return NextResponse.json({ sent: 0, message: 'No active teams' });
    }
      ...
      try {
        ...
        for (const m of rawMembers) {
          ...
          const row: AgentRow = {
            ...
            email: user?.email ?? '',
            role: m.role === 'manager_owner' ? 'Owner' : m.role === 'manager_admin' ? 'Admin' : 'Rep',
        ...
        console.error('[cron/manager-weekly-report] Failed for team', {
          teamId: team.id,
          error: err,
        });
      }
    ...
    <tr><td align="center">
      <table width="100%" cellpadding="0" cellspacing="0" style="max-width:640px;background:#ffffff;border-radius:12px;border:1px solid #e5e7eb;overflow:hidden">
        ...
        <tr><td style="background:#0f172a;padding:20px 28px">
          <p style="margin:0;color:#94a3b8;font-size:12px;font-weight:500;text-transform:uppercase;letter-spacing:.05em">${esc(teamName)}</p>
        ...
        <tr><td style="padding:16px 28px;border-top:1px solid #f1f5f9">
          <p style="margin:0;font-size:11px;color:#9ca3af;text-align:center">
            This weekly report is automatically generated for <strong>${esc(teamName)}</strong>. Manage your notification preferences in manager settings.
          </p>
        </td></tr>
      </table>
    </td></tr>
