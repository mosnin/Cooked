/**
 * `propose_demo_times` — suggest 4-6 open 60-minute slots on the rep's
 * calendar for the next 7 days during business hours. The chat renders
 * these as a tappable picker; the rep picks one → the workspace sends
 * a "Schedule the demo at <slot>" prompt → Koala fires `schedule_demo`
...
const parameters = z
  .object({
    ...
    propertyAddress: z
      ...
      .min(1)
      .optional()
      .describe('Property address the demo is for. Surface on the picker.'),
    durationMinutes: z
      .number()
      .int()
      .min(15)
  ...
  .describe(
    'Propose 4-6 open slots in business hours over the next week. Renders as a tappable picker — the rep picks a slot to fire schedule_demo.',
  );
...
/** Business hours window in the rep's local time. Lean toward
...
export const proposeDemoTimesTool = defineTool<typeof parameters, ProposeDemoTimesResult>({
  ...
  riskLevel: 'safe',
  description:
    'Suggest 4-6 open demo times for the rep over the next week. Returns slots the rep can tap to schedule.',
  parameters,
