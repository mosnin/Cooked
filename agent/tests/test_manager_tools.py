...
Phase 5 of Koala-for-Managers — defends three invariants:

  1. Every tool calls `require_manager_role(ctx)` first.
  2. Cross-team scope is enforced where applicable.
...
_ALL_TOOL_INPUTS: list[tuple[str, dict[str, Any]]] = [
    ("team_health", {}),
    ("rep_performance", {"rep_id": "u_rep"}),
    ("read_rep_morning_story", {"rep_id": "u_rep"}),
    ("find_stuck_deals", {}),
    ...
    ("find_at_risk_agents", {}),
    ("reassign_lead", {"lead_id": "c_lead", "to_rep_id": "u_rep"}),
...
async def test_non_manager_ctx_is_refused(tool_name: str, args: dict[str, Any]) -> None:
    ...
    ctx = _ctx(manager_role="rep_member")
...
@pytest.mark.asyncio
async def test_rep_performance_refuses_cross_team(fake_db: _FakeSupabase) -> None:
    """`rep_performance` runs against a rep whose Space lives in a
    DIFFERENT team — must refuse with ManagerPermissionError."""
    fake_db.set_for(
        "Space",
        "select",
        _FakeResult(
            data={"id": "s_other", "ownerId": "u_rep", "teamId": "bk_OTHER", "name": "Other Team"}
        ),
    )
    tool = _tool_by_name("rep_performance")
    ctx = _ctx(team_id="bk_a")
    with pytest.raises(ManagerPermissionError):
        await _invoke(tool, ctx, {"rep_id": "u_rep"})


@pytest.mark.asyncio
async def test_read_rep_morning_story_refuses_cross_team(fake_db: _FakeSupabase) -> None:
    """Same cross-team refusal on `read_rep_morning_story`."""
    fake_db.set_for(
        "Space",
        "select",
        _FakeResult(data={"id": "s_other", "ownerId": "u_rep", "teamId": "bk_OTHER"}),
    )
    tool = _tool_by_name("read_rep_morning_story")
    ctx = _ctx(team_id="bk_a")
    with pytest.raises(ManagerPermissionError):
        await _invoke(tool, ctx, {"rep_id": "u_rep"})
...
@pytest.mark.asyncio
async def test_reassign_lead_refuses_cross_team_contact(fake_db: _FakeSupabase) -> None:
    """`reassign_lead` returns ok=false when the contact is missing for this
    team. The TS path scopes contact lookup to team_id, and falls
    back to the manager owner's space — both lookups should miss for a
    cross-team lead. We mock contact lookup as None and ensure the
    Team row in `_resolve_manager_space` also returns no useful space.
    """
    # First contact lookup (by teamId): miss.
    fake_db.set_for("Contact", "select", _FakeResult(data=None))
    # `_resolve_manager_space` then queries Team; return a row but with
    ...
    fake_db.set_for(
        "Space",
        "select",
        _FakeResult(data={"id": "s_manager", "ownerId": "u_manager", "name": "Manager Space", "teamId": "bk_a"}),
    )
    ...
    ctx = _ctx(team_id="bk_a")
    res = await _invoke(tool, ctx, {"lead_id": "c_other", "to_rep_id": "u_rep"})
    assert res["ok"] is False
    assert "team" in res["summary"].lower() or "not found" in res["summary"].lower()
...
async def test_change_member_role_first_call_requires_confirmation(fake_db: _FakeSupabase) -> None:
    ...
    fake_db.set_for(
        ...
        "select",
        _FakeResult(
            data={"id": "m1", "userId": "u_rep", "role": "rep_member", "teamId": "bk_a"}
        ),
    )
    fake_db.set_for(
        "User",
        "select",
        _FakeResult(data={"id": "u_rep", "name": "Alice", "email": "alice@x.com"}),
    )
...
async def test_change_member_role_confirmed_executes(fake_db: _FakeSupabase) -> None:
    ...
    assert any(c["table"] == "TeamMembership" and c["op"] == "update" for c in fake_db.log)
...
async def test_offboard_member_first_call_requires_confirmation(fake_db: _FakeSupabase) -> None:
    ...
    fake_db.set_for(
        "User",
        "select",
        _FakeResult(data={"id": "u_rep", "name": "Alice", "email": "alice@x.com", "status": "active"}),
    )
...
async def test_offboard_member_confirmed_calls_rpc(fake_db: _FakeSupabase) -> None:
    """Confirmed call fires the offboard_team_member RPC and audits."""
    # Target membership.
    fake_db.set_for(
        "TeamMembership",
        "select",
        ...
        _FakeResult(
            data={"id": "m2", "userId": "u_dest", "role": "rep_member", "teamId": "bk_a"}
        ),
    )
    fake_db.set_for(
        "User",
    ...
    assert rpc_calls[0]["payload"]["fn"] == "offboard_team_member"
    assert any(c["table"] == "AuditLog" and c["op"] == "insert" for c in fake_db.log)
...
async def test_reassign_lead_writes_audit(fake_db: _FakeSupabase) -> None:
    ...
    # Contact lookup by teamId succeeds.
    fake_db.set_for(
        "Contact",
        "select",
        _FakeResult(
    ...
    # Space lookups: manager's space, target rep's space.
    fake_db.set_for(
        "Space",
        "select",
        _FakeResult(data={"id": "s_manager", "ownerId": "u_manager", "name": "Manager", "teamId": "bk_a"}),
        _FakeResult(data={"id": "s_rep", "ownerId": "u_rep", "name": "Rep", "teamId": "bk_a"}),
    )
    # Membership for target rep.
    fake_db.set_for(
        "TeamMembership",
        "select",
        _FakeResult(
            data={"id": "m_rep", "userId": "u_rep", "role": "rep_member"}
        ),
    )
    # Target user.
    fake_db.set_for(
        "User",
        "select",
        _FakeResult(data={"id": "u_rep", "name": "Jordan", "email": "j@x.com"}),
    )
    ...
    res = await _invoke(
        tool,
        ctx,
        {"lead_id": "c1", "to_rep_id": "u_rep", "reason": "lighter load"},
    )
    assert res["ok"] is True
    audit_inserts = [
...
async def test_flag_deal_writes_audit(fake_db: _FakeSupabase) -> None:
    fake_db.set_for(
        "Deal",
        "select",
        _FakeResult(data={"id": "d1", "title": "Sarah Chen — 123 Main", "spaceId": "s_rep", "status": "active"}),
    )
    fake_db.set_for(
        "Space",
        "select",
        _FakeResult(data={"id": "s_rep", "ownerId": "u_rep", "teamId": "bk_a"}),
    )
    ...
    fake_db.set_for(
        ...
        _FakeResult(
            data=[
                ...
                {"id": "m2", "userId": "u_rep", "role": "rep_member"},
            ]
        ),
    )
...
async def test_set_routing_rule_writes_audit(fake_db: _FakeSupabase) -> None:
    ...
    audits = [c for c in fake_db.log if c["table"] == "AuditLog" and c["op"] == "insert"]
    assert len(audits) == 1
    assert audits[0]["payload"]["resource"] == "Team"
    ...
    updates = [c for c in fake_db.log if c["table"] == "Team" and c["op"] == "update"]
...
# ── 6. manager_admin can only move rep_member ⇄ rep_member ───────────
...
async def test_change_member_role_admin_cannot_change_other_admins(
    ...
    rep_member — that move is owner-only."""
    fake_db.set_for(
        "TeamMembership",
        "select",
        _FakeResult(
            data={"id": "m_other_admin", "userId": "u_admin2", "role": "manager_admin", "teamId": "bk_a"}
        ),
    )
    fake_db.set_for(
        "User",
    ...
    res = await _invoke(
        tool,
        ctx,
        {"member_id": "m_other_admin", "new_role": "rep_member", "confirmed": True},
    )
    assert res["ok"] is False
    assert "owner" in res["summary"].lower()
    # No UPDATE happened — admin attempted owner-only move.
    assert not any(c["table"] == "TeamMembership" and c["op"] == "update" for c in fake_db.log)
...
async def test_change_member_role_admin_cannot_promote_to_admin(
    ...
    """A manager_admin cannot promote a rep_member to manager_admin
    either — promotion-to-admin is owner-only."""
    fake_db.set_for(
        "TeamMembership",
        "select",
        _FakeResult(
            data={"id": "m_rep", "userId": "u_rep", "role": "rep_member", "teamId": "bk_a"}
        ),
    )
    fake_db.set_for(
        "User",
    ...
    res = await _invoke(
        tool,
        ctx,
        {"member_id": "m_rep", "new_role": "manager_admin", "confirmed": True},
    )
    assert res["ok"] is False
    assert "owner" in res["summary"].lower()
...
async def test_set_routing_rule_rejects_invalid_strategy(fake_db: _FakeSupabase) -> None:
    ...
    # No UPDATE attempted.
    assert not any(c["table"] == "Team" and c["op"] == "update" for c in fake_db.log)
