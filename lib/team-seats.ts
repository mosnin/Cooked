/**
 * Team seat-limit helpers (BP3b).
 *
 * "Seats in use" for a team is defined as:
 *   members (rows in TeamMembership)
 *   + pending, non-expired invitations (Invitation.status='pending' AND expiresAt > now())
 *
 * Seat limits are driven off the Team.plan / Team.seatLimit columns.
...
export type TeamPlan = 'team' | 'team_plus';
...
const DEFAULT_PLAN: TeamPlan = 'team';
const DEFAULT_SEAT_LIMIT = PLANS.team.includedUsers;

function isValidPlan(value: unknown): value is TeamPlan {
  return value === 'team' || value === 'team_plus';
}

/**
 * Load plan + seatLimit for a team with pre-migration resilience.
...
async function loadPlan(
  teamId: string
): Promise<{ plan: TeamPlan; seatLimit: number | null }> {
  try {
    ...
    const plan: TeamPlan = isValidPlan(row.plan) ? row.plan : DEFAULT_PLAN;
    ...
    if (row.seatLimit === null || row.seatLimit === undefined) {
      ...
      // team tier, so we never fall back to null here.
...
/**
 * Count TeamMembership rows for a team.
 * Returns null on error so the caller can decide to fail-open.
 */
async function countMembers(teamId: string): Promise<number | null> {
  try {
...
/**
 * Count pending, non-expired invitations for a team.
 * Returns null on error so the caller can decide to fail-open.
 */
async function countPendingInvites(teamId: string): Promise<number | null> {
  try {
    const { count, error } = await supabase
      .from('Invitation')
      .select('*', { count: 'exact', head: true })
...
/**
 * Resolve current seat usage (plan + members + pending invites) for a team.
 ...
 */
export async function getSeatUsage(teamId: string): Promise<SeatUsage> {
...
/**
 * Check whether `additional` new seats can be added to a team.
 *
 * Rules:
 ...
 * (pre-migration) doesn't unlock the team.
 */
