 ...
 * when the rep taps a compose action. Replaces the chat-prefill
 * teleporter — Phase 7. The rep never leaves the home.
 *
...
interface Props {
  ...
  onSent: () => void;
  /** Called when the rep taps Cancel. */
  onCancel: () => void;
}
...
export function MorningActionSheet({ slug, intent, context, onSent, onCancel }: Props) {
  ...
  // reopening the panel re-drafts (the rep can re-roll by re-tapping).
  useEffect(() => {
  ...
  async function handleSend() {
    ...
    try {
      ...
      // The "not_configured" path is a half-win — the rep's intent was
  ...
  function handleEdit() {
    if (!draft) return;
    // The escape hatch — drop the rep into the chat with the draft as
