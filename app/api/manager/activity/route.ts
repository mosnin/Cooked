 ...
 *
 * Returns the team-scoped slice of the AuditLog table for compliance
...
export async function GET(req: NextRequest) {
  ...
  // If the team owns zero spaces we skip query A entirely — `.in('spaceId',
  ...
  // ── Query B: team-wide null-space rows ───────────────────────────────
  // Match via metadata->>'teamId' — safe because it's a strict equality
  // filter tied to the caller's team id. Rows that lack this metadata
  ...
  let nullSpaceResult: AuditLogRow[] = [];
  {
    let q = supabase
      ...
      .select('id, clerkId, ipAddress, action, resource, resourceId, spaceId, metadata, createdAt')
      .is('spaceId', null)
      .eq('metadata->>teamId', ctx.team.id)
      .gte('createdAt', sinceIso)
      .order('createdAt', { ascending: false })
      .order('id', { ascending: false })
      .limit(pageSize);
