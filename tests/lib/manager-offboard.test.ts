...
// `TeamMembership` table (target, then destination), so we let each table
...
// ── requireManager mock ─────────────────────────────────────────────────────
let requireManagerImpl: () => Promise<{ team: Team; membership: TeamMembership; dbUserId: string }>;
...
function makeTeam(overrides: Partial<Team> = {}): Team {
  return {
    ...
    teamFormConfig: null,
    teamRentalFormConfig: null,
    teamBuyerFormConfig: null,
    teamRentalScoringModel: null,
    teamBuyerScoringModel: null,
    plan: 'team',
    seatLimit: 5,
    ...
    teamLicenseNumber: null,
    teamFairHousingNotice: null,
    teamShowEqualHousingMark: false,
    ...overrides,
  };
}
...
function setCaller(role: 'manager_owner' | 'manager_admin' | 'rep_member' = 'manager_owner', dbUserId = 'u_caller'): void {
...
describe('POST /api/manager/members/[id]/offboard', () => {
  ...
  describe('lookups', () => {
    it('returns 404 when the target membership is not in this team', async () => {
      queueSingle('TeamMembership', null); // target not found
    ...
    it('returns 404 when the destination membership is not found', async () => {
      ...
      queueSingle('TeamMembership', null); // destination missing
  ...
  describe('happy paths', () => {
    ...
    it('dry-run returns counts plus leaving/destination names', async () => {
      ...
      expect((rpcMock.mock.calls as unknown[][])[0][0]).toBe('offboard_team_member');
    ...
    it('real-run returns moved counts and fires an OFFBOARD audit event', async () => {
      ...
      expect(auditArgs.resource).toBe('TeamMembership');
