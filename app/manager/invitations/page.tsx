...
const roleLabel = (role: string) =>
  role === 'manager_admin' ? 'Admin' : 'Rep';

// Role pill — small caps, muted bg. Matches the Members + Reps rows so
...
export default async function ManagerInvitationsPage() {
  ...
  const subtitle = (() => {
    if (invs.length === 0)
      return `Nobody invited to ${ctx.team.name} yet.`;
    if (pendingCount > 0) {
