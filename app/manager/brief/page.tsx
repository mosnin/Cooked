...
export default async function ManagerBriefPage() {
  ...
  // rep_member sees their own work surface, not the swarm.
  if (ctx.membership.role === 'rep_member') {
    return <MemberDashboard ctx={ctx} />;
  }

  const { team } = ctx;
  const members = await getTeamMembers(ctx.team.id, {
    includeOnboard: true,
  ...
  // Make sure the manager owner's space is included — team leads land
  ...
  const [
    ...
    draftStatsRows,
      ...
      // Same shape `aggregateDraftStats` expects; team-wide rollup.
      spaceIds.length > 0
        ? supabase
            .from('AgentDraft')
  ...
  if (topSpaceId) {
    ...
    const name = owner?.User?.name?.split(' ')[0] ?? owner?.User?.name ?? 'Rep';
    topRepMtd = { name, amount: topAmount };
  }
  ...
  const b = team as unknown as { defaultManagerRate?: number | null };
  ...
  if (team.slaEnabled && spaceIds.length > 0) {
    ...
    const [needsRes, escalatedRes] = await Promise.all([
      ...
      // Escalated today: ManagerNotification rows Koala created today for this team.
      supabase
  ...
  const hasSettings = !!(team.name && (team.logoUrl || team.websiteUrl));

  // Sort members by current activity load (active drafts + new leads + active deals)
  // so the manager scans the busiest reps first. Owner row stays pinned to top.
    ...
    <div className="max-w-5xl mx-auto space-y-12 pb-12">
      ...
      <header className="space-y-1.5">
        <p className="text-sm text-muted-foreground">
          {getGreeting()}. {team.name}.
        </p>
        <ManagerMorningStory />
      </header>

      {/* Koala — the focal entry. The manager's chief of staff is the home of
          this page, not its fourth section. A calm, paper-flat doorway: serif
          name, one line of what it does, a quiet pill into /manager/koala.
      ...
      <section>
        ...
        <div className="grid grid-cols-4 gap-px bg-border/70 rounded-xl overflow-hidden border border-border/70 mt-4">
          ...
          <div className="bg-background p-4">
            <p className={SECTION_LABEL}>Top rep (MTD)</p>
            <p
              className="text-2xl tracking-tight mt-1.5 text-foreground truncate"
              style={TITLE_FONT}
            >
              {topRepMtd ? topRepMtd.name : <span className="text-muted-foreground">—</span>}
            </p>
            {topRepMtd && (
              <p className="text-[11px] text-muted-foreground tabular-nums mt-1">
                {formatCompact(topRepMtd.amount)}
              </p>
            )}
          </div>
          <div className="bg-background p-4">
        ...
        {memberRows.length === 0 ? (
          <div className="mt-4 rounded-xl border border-dashed border-border/70 bg-muted/20 px-5 py-10 text-center">
            ...
            <p className="text-xs text-muted-foreground mt-1">
              Invite your first rep and their work will land in this view.
            </p>
          </div>
        ) : (
          <ul className="divide-y divide-border/60">
            {memberRows.map(({ member, leads, deals, drafts, apps }) => {
              const name = member.User?.name ?? 'Unnamed rep';
              const initial = name.charAt(0).toUpperCase();
              // A teammate row opens the manager-scoped rep detail page.
              // /s/<slug>/* requires owning that space, so a manager can't open a
              // teammate's own dashboard — it 404s. /manager/reps/<userId> is
              // the manager's view of that rep (the same link the Reps
              // list uses). The manager's own chief of staff lives at /manager/koala.
              const href = member.userId ? `/manager/reps/${member.userId}` : '#';
              const role =
      ...
      {/* Draft impact — how Koala's drafts have actually been landing across
          ...
          rep row remains the focal element of the page. */}
