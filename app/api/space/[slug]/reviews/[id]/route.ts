 ...
 * doesn't own, even within the same team.
 */
export async function GET(_req: NextRequest, { params }: Params) {
  ...
  const { data: row, error } = await supabase
    ...
    .select(
      'id, dealId, status, reason, createdAt, resolvedAt, resolvedByUserId, resolvedNote, requestingUserId, teamId',
    )
    .eq('id', reviewId)
  ...
  // requestingUserId and teamId; the latter defends against a freak
  // cross-team collision where the agent changed teams.
  if (
    !row ||
    row.requestingUserId !== dbUser.id ||
    row.teamId !== space.teamId
  ) {
    return NextResponse.json({ error: 'Review not found' }, { status: 404 });
  }
