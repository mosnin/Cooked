...
describe('POST /api/ai/task — reserved-title conversationId is not reused (isolation guard, #303)', () => {
  // A manager_owner also owns their personal rep space, and the pre-#295
  ...
  // conversationId (same spaceId but [MANAGER_KOALA]/[TEAM_CHAT] title) so
  // rep turns never append to — or read history from — a manager conversation.
  it('mints a fresh conversation instead of reusing a [MANAGER_KOALA] one', async () => {
    delete process.env.KOALA_CHAT_RUNTIME;
    convLookup.row = { id: 'manager_conv_1', spaceId: 's_1', title: '[MANAGER_KOALA] private notes' };
  ...
  it('mints a fresh conversation instead of reusing a [TEAM_CHAT] one', async () => {
    delete process.env.KOALA_CHAT_RUNTIME;
    convLookup.row = { id: 'team_conv_1', spaceId: 's_1', title: '[TEAM_CHAT] standup' };
  ...
  it('reuses a plain rep conversation in the same space (guard is not over-broad)', async () => {
    delete process.env.KOALA_CHAT_RUNTIME;
    convLookup.row = { id: 'rep_conv_1', spaceId: 's_1', title: 'Follow up with the Garcias' };
    await POST(makeRequest({ message: 'add Preston as a contact', conversationId: 'rep_conv_1' }));
    const call = tsStreamMock.mock.calls[0][0] as { conversationId: string };
    expect(call.conversationId).toBe('rep_conv_1');
  });
});
...
describe('POST /api/ai/task — dual-path router', () => {
  ...
  it('explicit mode:chat forces the direct path even for an action verb', async () => {
    ...
    // rep asked for a fast answer, not the tool loop.
  ...
  it('explicit mode:agent forces Modal even for a generic question', async () => {
    ...
    // acting runtime) so the rep's chosen runtime is honored.
  ...
  it('mode:agent degrades to the in-process TS agent when MODAL_CHAT_URL is unset', async () => {
    delete process.env.KOALA_CHAT_RUNTIME;
    delete process.env.MODAL_CHAT_URL;
    // A missing Modal URL must not fail the one turn the rep asked to run;
