...
// ── Shared types / schema ─────────────────────────────────────────────────────
//
// These shapes mirror the `TeamTemplate` table introduced in Phase BP6a.
...
// ── GET — any manager member may read their team's templates ──────────────

/**
 * GET /api/manager/templates
 *
 * Returns the team's template library ordered by updatedAt DESC.
 * All manager-scoped members (owner, admin, rep) can read — viewing the
...
export async function GET(): Promise<NextResponse> {
  ...
  return NextResponse.json((data ?? []) as TeamTemplateRow[]);
}
 ...
 * Restricted to manager_owner / manager_admin. Rep members may read the
