-- ============================================================================
-- CommissionLedger: retain the team's commission books when an agent's
...
-- financial records the team owns. docs/DATA-DELETION.md states these
-- records are meant to be RETAINED ("erasing one agent's account should not
-- destroy the team's commission books"); the FK contradicted that intent.
--
-- FIX: make agentUserId nullable and switch the FK to ON DELETE SET NULL. A
-- deleted agent's ledger rows survive with agentUserId = NULL (the team's
-- books stay intact; the row simply no longer points at a live User). The
-- teamId FK keeps ON DELETE CASCADE — deleting the whole team still
