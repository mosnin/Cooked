...
import type { RepStats } from './page';
...
export function LeaderboardClient({ initialStats }: { initialStats: RepStats[] }) {
      ...
      {sorted.length === 0 ? (
        <div className="rounded-xl border border-dashed border-border/70 bg-muted/20 px-4 py-6 text-center">
          <p className={cn(BODY_MUTED, 'text-[13px]')}>
            Nothing to rank yet — once reps close a deal I&apos;ll start the leaderboard.
          </p>
        </div>
      ) : (
