...
export default async function LeaderboardPage() {
    ...
    return (
      <div className="space-y-8 max-w-5xl mx-auto pb-56 md:pb-24">
        <header className="space-y-1.5">
          ...
          <p className={cn(BODY_MUTED)}>No reps to rank yet.</p>
        </header>
  ...
  const statusSentence = (() => {
    if (ranked === 0) return 'No reps to rank yet.';
    if (closed === 0) return `Ranking ${ranked} ${ranked === 1 ? 'rep' : 'reps'} — no deals closed yet.`;
    return `Ranking ${ranked} ${ranked === 1 ? 'rep' : 'reps'} · ${closed} ${closed === 1 ? 'deal' : 'deals'} closed.`;
  })();
