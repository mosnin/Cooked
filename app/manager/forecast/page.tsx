...
export const metadata: Metadata = { title: 'Revenue Forecast — Team' };
...
// set; otherwise fall back to the team's defaultManagerRate (same proxy
...
//   1. deal.commissionRate (set per-deal by the rep)
//   2. team.defaultManagerRate (team-wide default, ~ 2%)
...
export default async function ManagerForecastPage() {
  ...
  // Space to rep name lookup
  const spaceToRep = new Map<string, string>();
  for (const m of allMembers) {
    const sid = m.Space?.id;
    if (!sid) continue;
    spaceToRep.set(sid, m.User?.name ?? m.User?.email ?? 'Unknown');
  }
  ...
  if (spaceIds.length === 0) {
    return <EmptyPage teamName={team.name} monthName={monthName} />;
  }
  ...
  // Fetch closed deals (won + lost, all time) for per-rep conversion and
  ...
  // the per-rep rollup sees the full pipeline, not just the closed tail.
  ...
  let wonGci = 0;
  let wonCount = 0;
  // Realized GCI this month, per rep space — feeds the per-rep rollup.
  ...
  for (const d of (activeRaw ?? []) as DealRow[]) {
    ...
    const repName = spaceToRep.get(d.spaceId) ?? 'Unknown';
  ...
  // ── Per-rep performance context ────────────────────────────────────────
  // For each rep space we fold together: their active deals (from
  ...
  type RepMetricRow = DealMetricRow & { spaceId: string };

  const metricRowsBySpace = new Map<string, RepMetricRow[]>();
  const pushRow = (row: RepMetricRow) => {
  ...
  const repRollups: RepRollup[] = Array.from(repSpaceIds)
    ...
    // Show reps with anything in flight or realized this month first; drop
  ...
  // Run across the team's active deals. Stages come from the same set we
  ...
  return (
    <div className={cn('max-w-5xl mx-auto pb-12', SECTION_RHYTHM)}>
      ...
      <header className="space-y-1.5">
        <p className={BODY_MUTED}>{team.name}.</p>
      ...
      {!hasAnything ? (
        ...
        <>
          ...
          <section>
            ...
            {swingPool.length === 0 ? (
              ...
              <ul className="divide-y divide-border/60">
                {swingPool.map((deal) => {
                  const koalaPrompt = `What can we do to close "${deal.title}" (${deal.repName}) this month?`;
                  const koalaHref = `/manager?prompt=${encodeURIComponent(koalaPrompt)}`;
                      ...
                      <div className="flex items-start justify-between gap-4">
                        ...
                        <div className="flex-1 min-w-0 space-y-1">
                          ...
                          <div className="flex items-center flex-wrap gap-x-3 gap-y-0.5">
                            <span className={META}>{deal.repName}</span>
          ...
          <section>
            <div className="flex items-center gap-3 pb-3 border-b border-border/60">
              <h2 className={SECTION_LABEL}>By rep</h2>
              {repRollups.length > 0 && (
                <span className="text-[11px] text-muted-foreground tabular-nums">
                  {repRollups.length}
                </span>
            ...
            {repRollups.length === 0 ? (
              <div className="rounded-xl border border-dashed border-border/70 bg-muted/20 px-5 py-10 text-center mt-4">
                <p className="text-sm text-foreground">No rep activity yet.</p>
                <p className="text-xs text-muted-foreground mt-1">
                  Each rep&rsquo;s forecast and pace will appear here as deals move.
                </p>
              </div>
            ) : (
              <ul className="divide-y divide-border/60">
          ...
          <p className={cn(CAPTION, 'text-muted-foreground/70')}>
            ...
            5% by health. Commission source: per-deal rate when set, otherwise team default
...
// ── Empty state when team has no members/spaces ─────────────────────────
...
function EmptyPage({
  ...
  return (
    <div className={cn('max-w-5xl mx-auto pb-12', SECTION_RHYTHM)}>
      <header className="space-y-1.5">
        <p className={BODY_MUTED}>{teamName}.</p>
        <h1 className={H1} style={TITLE_FONT}>
          {monthName} revenue
        </h1>
      ...
      <div className="rounded-xl border border-dashed border-border/70 bg-muted/20 px-5 py-10 text-center">
        <p className="text-sm text-foreground">Nothing in the pipeline yet.</p>
        <p className="text-xs text-muted-foreground mt-1">
          Invite your first rep and their deals will appear here.
        </p>
      </div>
    </div>
  );
