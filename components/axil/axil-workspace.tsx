...
interface KoalaWorkspaceProps {
   ...
   *  line under the post-demo affordance. Snapshot at page load: the rep
...
export function KoalaWorkspace({
  ...
  // The single sentence the rep sees when they approve a celebrate-able
  ...
  const {
    ...
    ...(isManager
      ? {
          ...
          // The manager conversations route reads team scope from the
  ...
  // Wrap approve / alwaysAllow so the moment the rep approves a
  ...
  // (reads "ManagerMessage"); rep history from `/api/ai/messages` (reads
  ...
  // 404 — the rep endpoint refuses manager conversations by design.
  ...
  // `/api/ai/manager-conversations/<id>` for the manager variant — the rep
  ...
  // call is delayed enough that the rep can walk it back without a
  ...
  useEffect(() => {
    return () => {
      ...
      // a page nav after the rep walked away from the toast.
  ...
  async function handleDeleteConversation(id: string) {
    ...
    // Optimistic: remove from the sidebar immediately so the rep sees
    ...
    // (which scopes the delete to the caller's team). The rep route
    // is used for the rep variant.
    const timer = setTimeout(async () => {
      ...
      try {
        ...
        if (!res.ok) {
          ...
          // rep. They've already moved on by now, so the toast carries
  ...
  const handleMentionSearch = useCallback(
    async (query: string): Promise<MentionItem[]> => {
      ...
      try {
        ...
        // id; once a search succeeds again the rep just dismisses it.
  ...
  const handleSend = useCallback(
    async (
      ...
      // clean — the helper no-ops when the rep hasn't enabled sound
  ...
  // (the server doesn't know the rep's local hour); we render an
  ...
  // those endpoints are space-scoped (rep-only). Skip the fetch.
  ...
  // Day-one signal: zero of everything. The rep has truly never engaged.
    ...
    if (liveCallIds && liveCallIds.size > 0) {
      for (const block of tailMessage.blocks) {
        if (
          ...
          // get a friendly verb derived from the toolkit name so reps
    ...
    <div className="relative flex flex-col h-full min-h-0">
      ...
      <div className="absolute top-1.5 right-2 sm:top-2 sm:right-3 z-20 flex items-center gap-1.5">
        ...
        {/* Split panel disabled on manager variant — RightPanel tabs
            (contacts/deals/etc.) are rep-scoped in Phase 1. */}
            ...
            >
              ...
              <div className="w-full max-w-2xl">
                <motion.h1
                  ...
                  {/* Day-one guidance — rep only, and only for users with
                      zero history. The manager chat goes straight from greeting
                      to composer (no subtext) per the team design. */}
                  {!isManager && isFresh && (
                    <motion.p
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0 pt-0.5 space-y-3">
                            ...
                            {/* Inline retry button — shown on the tail error
                                message so the rep doesn't have to retype. */}
