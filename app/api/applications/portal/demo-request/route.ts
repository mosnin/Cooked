 ...
 *
 * Why a structured request and not "just a chat message" — the rep's
...
export async function POST(req: NextRequest) {
  ...
  // Compose the AgentQuestion that surfaces in the rep's Koala focus
  ...
  const questionInsert = supabase
    ...
    .insert({
      ...
      context: [
        ...
        .join('\n'),
      status: 'pending',
      priority: 50, // mid-high — applicant action; rep should see it today
      contactId: contact.id,
    })
    .select('id')
    .single();
  ...
  if (questionRes.error) {
    // Soft-fail the AgentQuestion — the message landed, the rep sees it.
    // Log for observability but don't 500 the user.
    console.error('[portal/demo-request] Question insert error:', questionRes.error);
  }

  // Notify rep (fire and forget — same pattern as message endpoint).
  void notifyRepOfDemoRequest(
    contact.spaceId,
    contact.name,
    safeTimes,
    safeAddress,
    ...
    console.error('[portal/demo-request] Rep notification failed:', err),
  );
...
/**
 * Email the rep about a new demo request. Same template family as
 * portal/message's notification, with a clearer "demo requested" subject
 * and a structured body.
 */
async function notifyRepOfDemoRequest(
  spaceId: string,
