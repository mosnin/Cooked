...
import { RepsClient, type RepRow } from './reps-client';

export const metadata: Metadata = { title: 'Reps — Teams' };
...
export default async function ManagerRepsPage() {
  ...
  const members = await getTeamMembers(team.id, { includeOnboard: true, includeSpaceName: true });
  ...
  // Pull the full health picture per rep in one parallel volley:
  ...
  function bandFor(avgHours: number | null): RepRow['responseBand'] {
  ...
  const reps: RepRow[] = members.map((m) => {
  ...
  const subtitle = (() => {
    if (reps.length === 0) {
      return 'No reps yet. Send the first invite.';
    }
    const active = reps.filter((r) => r.onboard);
    if (active.length === 0) {
      return `${reps.length} invited. Nobody onboard yet.`;
    }
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto pb-56 md:pb-24">
      <header className="space-y-1.5">
        <p className={cn(BODY_MUTED)}>Reps.</p>
      ...
      {members.length === 0 ? (
        <div className="rounded-xl border border-dashed border-border/70 bg-muted/20 px-5 py-12 text-center">
          <p className="text-sm text-foreground">No reps yet.</p>
