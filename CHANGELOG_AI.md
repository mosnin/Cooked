...
- **Reason**: The old `/api/ai/chat` route was a non-tool-using LLM call that could not take action on behalf of the rep. The new agent can search, draft, and execute with an approval gate on every mutating step, while sub-agents stop read-heavy investigations from poisoning the main context window.
- **Risks**:
