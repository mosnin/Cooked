 ...
 *     one `{placeholder}` for the rep to fill in). A skill appears in
...
/** A skill the rep can invoke from the chat `/` menu. */
export interface UserSkill {
  slug: string;
  title: string;
  description: string;
  /** Composer text; may contain one `{placeholder}` for the rep to fill. */
  prompt: string;
}
